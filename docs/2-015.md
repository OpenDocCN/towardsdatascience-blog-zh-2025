...
> [`towardsdatascience.com/how-to-perform-comprehensive-large-scale-llm-validation/`](https://towardsdatascience.com/how-to-perform-comprehensive-large-scale-llm-validation/)

<mdspan datatext="el1755820801926" class="mdspan-comment">输出验证</mdspan>和评估对于确保稳健、高性能的 LLM 应用至关重要。然而，在 LLM 的更大框架中，这些主题往往被忽视。

想象这个场景：当被提示时，您的 LLM 查询正确回复了 999/1000 次。然而，您必须对 150 万个项目进行回填以填充数据库。在这个（非常现实）的场景中，您将仅为此 LLM 提示体验到**1500 个错误**。现在将这个规模扩大到 10 个，如果不是 100 个不同的提示，您将面临一个真正的可扩展性问题。

**解决方案**是验证您的 LLM 输出并确保通过评估获得高性能，这两个主题我将在本文中讨论。

![此信息图表突出了本文的主要内容。我将讨论 LLM 输出的验证和评估、定性 vs 定量评分，以及处理大规模 LLM 应用。](img/dedb5f934a1440fc79b006a198fe47d3.png)
...
您还可以阅读我的文章《如何使用 ARC AGI 3 基准测试 LLM》（[How to Benchmark LLMs using ARC AGI 3](https://towardsdatascience.com/how-to-benchmark-llms-arc-agi-3/)）。
...
+   使用[如 Tiktoken](https://pypi.org/project/tiktoken/)之类的库来计算请求中的令牌数量
...
![图片](img/c3c2da3cc4c927ab02f6a12905401386.png)
...
更高级且成本更高的验证方法是使用一个大型语言模型（LLM）。在这种情况下，你利用另一个 LLM 来评估输出是否有效。这是因为验证正确性通常比生成正确响应的任务更直接。使用 LLM 验证器本质上是将 LLM 作为裁判使用，关于这个话题，我在这篇《Towards Data Science》的文章中进行了讨论。[如何使用 LLM 进行强大的自动评估](https://towardsdatascience.com/how-to-use-llms-for-powerful-automatic-evaluations/)

我经常使用较小的 LLM 来执行此验证任务，因为它们有更快的响应时间，成本更低，而且考虑到验证的任务比生成正确响应的任务简单，它们仍然表现良好。例如，如果我使用[GPT-4.1](https://openai.com/index/gpt-4-1/)来生成摘要，我会考虑使用 GPT-4.1-mini 或 GPT-4.1-nano 来评估生成的摘要的有效性。
...
我建议详细说明评判标准给 LLM 评判者。例如，你应该说明 1 分、5 分和 10 分分别代表什么。使用例子通常是指导 LLM 的好方法，正如我在[关于如何利用 LLM 作为评判者的文章](https://towardsdatascience.com/how-to-use-llms-for-powerful-automatic-evaluations/)中讨论的那样。我经常思考当有人解释一个主题时，例子对我有多有帮助，因此您可以想象这对 LLM 有多有帮助。
...
您还可以阅读我的关于[如何使用 ARC AGI 3 基准测试 LLMs](https://towardsdatascience.com/how-to-benchmark-llms-arc-agi-3/)和[如何轻松使用 OCR 和 GPT-4o mini 提取收据信息](https://eivindkjosbakken.com/2025/08/19/how-to-effortlessly-extract-receipt-information-with-ocr-and-gpt-4o-mini/)的文章。
