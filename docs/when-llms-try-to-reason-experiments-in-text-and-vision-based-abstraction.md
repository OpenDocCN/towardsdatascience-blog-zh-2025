...
> 原文：[`towardsdatascience.com/when-llms-try-to-reason-experiments-in-text-and-vision-based-abstraction/`](https://towardsdatascience.com/when-llms-try-to-reason-experiments-in-text-and-vision-based-abstraction/)

*<mdspan datatext="el1753119493516" class="mdspan-comment">大型语言模型能否仅从少量示例中学习抽象推理？在这篇文章中，我通过测试基于文本的（o3-mini）和图像识别能力（gpt-4.1）模型在抽象网格转换任务上的表现来探讨这个问题。这些实验揭示了当前模型在多大程度上依赖于模式匹配、程序启发式和符号捷径，而不是稳健的泛化。即使有多模态输入，推理在面对微妙抽象时也常常失败。这些结果为 LLMs 在上下文元学习中的当前能力和局限性提供了一个窗口。>*

## 引言

元学习，即系统学习如何学习的能力，传统上通过基于梯度的优化、记忆增强网络或显式任务嵌入来探索。但随着大型语言模型（LLMs）的兴起，尤其是具有高级推理能力的 o3 系列，一个新的问题出现了：我们能否将 LLMs 本身作为基于任务的领域（如[ARC](https://arcprize.org/)）中的元学习器？由弗朗索瓦·肖莱特引入的抽象和推理语料库（ARC）是一个专门设计来测试广泛泛化的基准。它提供了最小监督的输入-输出转换谜题，每个任务只有少量示例，并且通常没有任务之间的共享表面结构。换句话说：一个元学习的游乐场。为了了解典型的抽象和推理任务，读者可以访问 ARC 游戏[页面](https://arcprize.org/play)。

![](img/d79c98b0f5e7917357e4a67b1dbaa65c.png)

来自 ARC [网站](https://arcprize.org/play)的[示例游戏](https://arcprize.org/play?task=00d62c1b)。从演示网格中可以看出，测试网格的任务是将黑色区域转换为黄色，只要它们完全被绿色边界包围。

## 数据和设置

为了探索 LLMs 如`o3-mini`是否能够在抽象推理任务上执行元学习，我使用了[ARC Prize 2025 Kaggle 比赛](https://www.kaggle.com/competitions/arc-prize-2025/overview)的数据。数据集存储库可以在[这里](https://github.com/arcprize/ARC-AGI-2)找到（Apache 2.0 许可证）。该数据集由输入-输出网格转换组成，挑战模型从少量示例中推断抽象规则。
...
每个网格是一个 0 到 9 的整数 2D 数组，代表彩色像素。网格大小各异，网格变换也可能从输入网格到输出网格的大小变化。为了可视化数组，我使用了`matplotlib`的自定义颜色映射：
...
对于模型交互，我通过 LangChain 使用了 OpenAI 的`o3-mini`模型。稍后，我们还将使用`gpt-4.1`：
...
我使用 LangChain 的`ChatPromptTemplate`定义了一系列提示，以模拟推理、应用、评估和改进。这个过程模仿了一个带有有限监督的内部训练循环：
...
在某些情况下，`o3-mini`能够仅从几个输入输出演示中正确推断出可泛化的算法。一个这样的例子是生成基于小 2×2 输入网格的图案平铺。

![图片](img/e56a8c42b1504533fc58e2bc711d2f5b.png)
...
![图片](img/d8f4c9c3231e01f9440f6aca72181aa2.png)

真实输出网格（图片由作者提供）。

![图片](img/c497dd3d6cb0de21bd7284e79a9ce33a.png)
...
![图片](img/1caf501f6456c246a75ea0b9355c3692.png)
...
![图片](img/38f1fa1f57d82fa188be93b625fa701f.png)

真实输出网格（图片由作者提供）。

![图片](img/90d1656d1257fd1fdc5e9233e5dd25d6.png)
...
![图片](img/beace9079b0849755fe1ebf091ec49cd.png)
...
![图片](img/37dd9b4f146e6bdc963be5d8ba20bb81.png)

预期测试输出网格（图片由作者提供）。

![图片](img/6ef7a55ec6bd8ab414c340b469e4149b.png)
...
![图片](img/7d8f913cb7d7e4dc333c3c5156a68058.png)
...
在这项研究中，我探讨了大型语言模型（无论是基于文本的还是多模态的）是否可以从抽象推理任务的例子中进行**元学习**。具体来说，我关注了来自 ARC 数据集的一类问题，其中解决方案需要识别视觉模式、学习转换并将它们推广到新的测试输入。
...
+   **多模态模型**（例如，具有视觉功能的 GPT-4）在**模式检测**和关系推理方面显示出**明显的改进**。它们成功地识别了蓝色边界的区域，并根据相对大小或嵌套调整行为。然而，它们的泛化仍然脆弱：它们仍然应用了脆弱的规则，如基于固定大小的分配，并在与演示不同的新布局中失败。
...
今天的 LLMs（大型语言模型）在广度上令人惊叹，但它们仍然是基于先验知识进行猜测，而不是在人类意义上*学习如何学习*。它们缺乏对抽象和转换的强大归纳偏见。ARC 风格的任务清楚地揭示了这一差距：成功需要的不只是模式识别，还需要以结构化、组合的方式从示例中进行*推理*。这些结果并不令人沮丧，反而更加明确。我们现在知道了天花板在哪里。下一代模型，那些具有混合架构、持久记忆和显式元学习能力的模型，可能最终会突破这一限制。
