# 开发 AI 驱动的商业规划与创业智能指南

> [`towardsdatascience.com/developing-an-ai-powered-smart-guide-for-business-planning-entrepreneurship-ea1bc328ff01/`](https://towardsdatascience.com/developing-an-ai-powered-smart-guide-for-business-planning-entrepreneurship-ea1bc328ff01/)
...
在 ChatGPT 发布以及随后大型语言模型（LLMs）的激增之后，它们固有的幻觉、知识截止日期以及无法提供组织或个人特定信息的局限性很快变得明显，并被视为主要缺点。为了解决这些问题，检索增强生成（RAG）方法很快得到了认可，这些方法将外部数据整合到 LLMs 中，并引导其行为以回答来自给定知识库的问题。

有趣的是，关于 RAG 的第一篇论文于 2020 年由 Facebook AI Research（现 Meta AI）的研究人员发表，但直到 ChatGPT 的出现，其潜力才得到充分实现。从那时起，就没有停止过。引入了更多先进和复杂的 RAG 框架，这不仅提高了该技术的准确性，还使其能够处理多模态数据，扩大了其在广泛应用中的潜力。我在以下文章中详细讨论了这一主题，特别是讨论了上下文多模态 RAG、多模态 AI 在商业应用中的搜索以及信息提取和匹配平台。 

> [**将多模态数据整合到大型语言模型中**](https://towardsdatascience.com/integrating-multimodal-data-into-a-large-language-model-d1965b8ab00c)
> 
> [**多模态 AI 在商业应用中的搜索**](https://towardsdatascience.com/multimodal-ai-search-for-business-applications-65356d011009)
> 
> [**AI 驱动的信息提取和匹配**](https://towardsdatascience.com/ai-powered-information-extraction-and-matchmaking-0408c93ec1b9)

随着 RAG 技术领域的扩展和新兴的数据访问需求，人们意识到仅提供检索功能的 RAG，即从静态知识库中回答问题的功能，可以通过整合其他多样化的知识来源和工具得到扩展，例如：

+   多个数据库（例如，包含向量数据库和知识图谱的知识库）

+   实时网络搜索以获取最新信息

+   外部 API 收集特定数据，如股市趋势或来自公司特定工具（如 Slack 频道或电子邮件账户）的数据

+   用于数据分析、报告撰写、文献综述和人物搜索等任务的工具。

+   比较和整合来自多个来源的信息。

![描述检索代理根据用户查询选择工具的情况（作者图片）](img/2df3e3e71ab0f7d030237ecd8f32e4b3.png)

描述检索代理根据用户查询选择工具的情况（作者图片）

要实现这一点，一个 RAG 应该能够根据查询选择最佳的知识来源和/或工具。AI 代理的出现引入了“*代理 RAG*”的概念，它可以根据查询选择最佳的行动方案。

在本文中，我们将开发一个特定的代理 RAG 应用，称为***智能商业指南（SBG）***——这是我们正在进行的项目[UPBEAT](https://www.haaga-helia.fi/fi/hankkeet/upbeat)的第一个版本，该项目由[Interreg Central Baltic](https://interreg.eu/programme/interreg-finland-estonia-latvia-sweden/)资助。该项目专注于利用 AI 提高芬兰和爱沙尼亚移民的创业和商业规划技能。SBG 是该项目提升技能过程中打算使用的工具之一。该工具专注于为打算创业或已经创业的人提供来自真实来源的精确和快速信息。

SBG 的代理 RAG 包括：

+   商业和创业指南作为包含有关商业规划、创业、公司注册、税收、商业想法、规则和法规、商业机会、许可证和执照、商业指南等信息的知识库。

+   通过网络搜索获取带有来源的最近信息。

+   知识提取工具从受信任的来源获取信息。这些信息包括相关当局的联系方式、最近的税收规则、最近的商业注册规则和最近的许可法规。

***这个代理 RAG 有什么特别之处？***

+   选择**不同开源模型**（Llama、Mistral、Gemma）以及**专有模型**（gpt-4o、gpt-4o-min_i）在整个代理工作流程中的选项。开源模型不本地运行，因此不需要强大的、昂贵的计算设备。相反，它们在**[Groq Cloud 的](https://console.groq.com/login)平台**上运行，并使用**免费 API**。是的，这使得它成为一个**免费代理 RAG**。也可以使用 OpenAI 的 API 密钥选择 GPT 模型。

+   强制执行知识库搜索、网络搜索和混合搜索的选项。

+   对检索到的文档进行评分以提高响应质量，并根据评分智能调用网络搜索。

+   选择响应类型的选项：**简洁**、**适中**或**解释性**。
...
1.  使用免费、开源模型开发高级代理 RAG（以下简称智能商业指南或 SBG）。

***该应用程序的完整代码可在[GitHub](https://github.com/umairalipathan1980/Smart-Agentic-Guide)上找到。***

应用程序代码以两个.*py*文件结构化：_agentic*rag.py*，该文件实现了整个代理工作流程，以及*app.py*，该文件实现了*Streamlit*图形用户界面。
...
[LlamaParse](https://docs.llamaindex.ai/en/stable/llama_cloud/llama_parse/)是一个专为 LLM 和 LLM 用例构建的 genAI 原生文档解析平台。我在上面引用的文章中解释了 LlamaParse 的使用。这次，我直接在[LlamaCloud](https://cloud.llamaindex.ai/)上解析了文档。LlamaParse 每天提供 1000 个免费积分。这些积分的使用取决于解析模式。对于纯文本 PDF，‘*快速*’模式（1 积分/3 页）效果良好，该模式跳过了 OCR、图像提取和表格/标题识别。还有其他更高级的模式，每页的积分点数更高。我选择了‘*高级*’模式，该模式执行 OCR、图像提取和表格/标题识别，非常适合包含图像的复杂文档。
...
从 LlamaCloud 下载了解析后的文档，格式为 Markdown。同样的解析也可以通过 LlamaCloud API 完成，如下所示。
...
下面是来自 Pikkala, A.等人所著的指南[创造力和商业](https://www.humak.fi/wp-content/uploads/2015/04/2015-04-Luovuus-ja-liiketoiminta-_tyokirja_englanti_netti.pdf)的一个示例页面（2015）的示例页面（“*免费复制用于非商业私人或公共用途，需注明出处*”）。

![来自商业指南的示例页面（来源：https://www.humak.fi/wp-content/uploads/2015/04/2015-04-Luovuus-ja-liiketoiminta-_tyokirja_englanti_netti.pdf)](img/b63b6d6f83bd57af8b527d1bb932129a.png)

以下是一个来自商业指南的示例页面（来源：[`www.humak.fi/wp-content/uploads/2015/04/2015-04-Luovuus-ja-liiketoiminta-_tyokirja_englanti_netti.pdf`](https://www.humak.fi/wp-content/uploads/2015/04/2015-04-Luovuus-ja-liiketoiminta-_tyokirja_englanti_netti.pdf)）

下面是这个页面的解析输出。LlamaParse 有效地从页面中的所有结构中提取了信息。页面中显示的笔记本是图像格式。
...
使用 LangChain 的*RecursiveCharacterTextSplitter*将解析后的 Markdown 文档分割成块，块大小为 CHUNK_SIZE = 3000，块重叠为 CHUNK_OVERLAP = 200。
...
随后，在 Chroma 数据库中使用嵌入模型（如开源的*all-MiniLM-L6-v2*模型或 OpenAI 的*text-embedding-3-large*）创建一个向量存储。
...
> 人工智能代理是工作流程和决策逻辑的组合，可以智能地回答问题或执行需要分解为更简单子任务的其他复杂任务。

我使用 LangGraph 为我们的 AI 代理设计了一个工作流程，该工作流程以图形的形式表示动作或决策的序列。我们的代理必须决定是否从向量数据库（知识库）、网络搜索、混合搜索或使用工具来回答问题。

在我接下来的文章中，我解释了使用 LangGraph 创建代理工作流程的过程。

> [**如何使用自动网络搜索开发免费 AI 代理**](https://ai.gopubby.com/how-to-develop-a-free-ai-agent-with-automatic-internet-search-5ea24928d26b)

我们需要创建代表决策工作流程的**节点**（例如，网络搜索或向量数据库搜索）。节点通过**边**连接，这些边定义了决策和动作的流程（例如，检索后的下一个状态是什么）。图**状态**跟踪信息在图中的移动，以便代理为每个步骤使用正确的数据。

工作流程的入口是一个路由函数，它通过分析用户的查询来确定工作流程中要执行的初始节点。整个工作流程包含以下节点。

+   ***检索***：从向量存储中检索语义上相似的信息块。

+   _**评估文档**_：根据用户的查询评估检索到的片段的相关性。

+   _**评估后路由**_：根据评估结果，确定是否使用检索到的文档生成响应或继续进行网络搜索。

+   ***网络搜索***：使用[Tavily](https://tavily.com/)搜索引擎的 API 从网络来源获取信息。

+   ***生成***：使用提供的上下文（从向量存储和/或网络搜索中检索到的信息）对用户的查询生成响应。

+   _**获取联系工具**_：从与芬兰移民服务相关的预定义受信任 URL 中获取联系信息。

+   _**获取税务信息**_：从预定义的受信任 URL 中获取与税务相关的信息。

+   _**获取注册信息**_：从预定义的受信任 URL 中获取芬兰公司注册过程的详细信息。

+   _**获取许可信息**_：获取在芬兰开设企业所需的许可证和许可信息。

+   _**混合搜索**_：结合文档检索和网络搜索结果，为回答查询提供更广泛的上下文。

+   ***无关***：处理与工作流程焦点无关的问题

这里是工作流程中的边缘。

+   _**检索 → 评估文档**_：检索到的文档将被发送进行评估。

+   _**评估文档 → 网络搜索**_：如果检索到的文档被认为是不相关的，则调用网络搜索。

+   **_grade_documents → generate***：如果检索到的文档相关，则继续进行响应生成。

+   **websearch → generate***：将网络搜索的结果传递给响应生成。

+   **_get_contact_tool, get_tax*info*, _get_registration*info*, _get_licensing*info → generate***：从这些四个工具到**generate**节点的边传递从特定可信来源获取的信息，用于响应生成。

+   **_hybrid*search* → *generate***：将组合结果（向量存储 + 网络搜索）传递给响应生成。

+   **unrelated* → *generate***：为无关问题提供回退响应。

图状态结构充当维护工作流程状态的容器，包括以下元素：

+   **question***：用户的查询或输入，驱动工作流程。

+   **生成**：对用户查询的最终生成响应，在处理之后填充。

+   **_web_search_needed**_：一个标志，指示是否需要基于检索到的文档的相关性进行网络搜索。

+   **documents***：与查询相关的检索或处理文档的列表。

+   **_answer_style**_：指定所需的答案风格，例如“简洁”、“适中”或“解释性”。
...
以下路由功能分析查询并将其路由到相关节点进行处理。创建一个链，包括从工具选择字典中选择工具/节点和查询的提示。该链调用路由 LLM 以选择相关工具。
...
与工作流程无关的问题会被路由到处理**无关**节点的**generate**节点，该节点提供回退响应。
...
**retrieve**节点调用检索器，用问题从向量存储中检索相关信息。这些信息块（“*documents*”）被发送到**grade_documents**节点以评估其相关性。根据评估的信息块（“_filtered*docs*”），**route_after_grading**节点决定是否使用检索到的信息继续生成，还是调用网络搜索。辅助函数**initialize_grader_chain**使用提示初始化评分链，引导评分 LLM 评估每个信息块的相关性。**grade_documents**节点分析每个信息块以确定它是否与问题相关。对于每个信息块，它输出“*Yes*”或“*No*”，取决于信息块是否与问题相关。
...
当检索到的信息中没有找到相关片段时，通过**route_after_grading**节点到达**web_search**节点，或者当**internet_search_enabled**状态标志为“*True*”（在用户界面中通过单选按钮选择）或路由器函数决定将查询路由到**web_search**以获取最新和更相关的信息时，可以直接通过**route_question**节点到达**web_search**节点。

通过在他们的网站上创建账户，可以获得 Tavily 搜索引擎的免费 API。免费计划每月提供 1000 个积分。Tavily 搜索结果附加到状态变量"*document*"，然后与状态变量"*question*"一起传递给**generate**节点。

混合搜索结合了检索器和 Tavily 搜索的结果，填充"*document*"状态变量，该变量随后与"*question*"状态变量一起传递给生成节点。
...
在此代理工作流程中使用的工具是抓取功能，用于从预定义的受信任 URL 中获取信息。Tavily 与这些工具的不同之处在于，Tavily 执行更广泛的互联网搜索，以从不同来源获取结果。而，这些工具使用 Python 的 Beautiful Soup 网络抓取库从受信任的来源（预定义的 URL）中提取信息。这样，我们确保有关特定查询的信息是从已知、受信任的来源提取的。此外，这种信息检索是完全免费的。

下面是如何使用一些辅助函数来工作**get_tax_info**节点。此类型的其他工具（节点）也以相同的方式工作。
...
**generate**节点通过调用一个包含预定义提示（以下描述的 LangChain 的**PromptTemplate**类）的链来创建最终响应。**rag_prompt**接收状态变量**"question"**、**"context"**和**"answer_style"**，并指导整个响应生成行为，包括有关响应风格、对话语气、格式指南、引用规则、混合上下文处理和仅关注上下文的说明。
...
**generate**节点首先检索状态变量"*question*"、"*documents*"和"_answer_style*"，并将"*documents*"格式化为单个字符串，作为上下文。随后，它使用**rag_prompt**和响应生成 LLM 来调用生成链，以生成最终答案，该答案填充在"*generation*"状态变量中。此状态变量由**app.py**用于在**Streamlit**用户界面中显示生成的响应。

使用 Groq 的免费 API，有可能达到模型速率或上下文窗口限制。在这种情况下，我扩展了**generate**节点，以循环方式从模型名称列表中动态切换模型，并在生成响应后返回到当前模型。
...
在 _agentic*rag.py* 中还有其他辅助函数用于初始化应用程序、LLM、嵌入模型和会话变量。在 *app.py* 中的应用程序初始化期间调用 _initialize*app* 函数，并且每次通过 *Streamlit* 应用程序更改模型或状态变量时都会触发 __。该函数重新初始化组件并保存更新后的状态。此函数还跟踪各种会话变量并防止重复初始化。
...
以下辅助函数初始化了一个回答 LLM、嵌入模型、路由 LLM 和评分 LLM。模型名称列表 _model*list* 用于在由 *generate* 节点动态切换模型时跟踪模型。
...
现在，定义了图状态、节点、使用 _route*question* 的条件入口点和边，以建立节点之间的流程。最后，将工作流程编译成可在 *Streamlit* 界面中使用的可执行 *app*。工作流程中的条件入口点使用 _route*question* 函数根据查询选择工作流程中的第一个节点。条件边 (_workflow.add_conditional*edges*) 描述了根据 _grade*documents* 节点确定的块的相关性，决定是否过渡到 *websearch* 或 *generate* 节点。
...
*app.py* 中的 Streamlit 应用程序提供了一个交互式界面，用于使用动态设置（模型选择、回答风格和针对特定查询的工具）提问并显示响应。从 _agentic*rag.py* 导入的 _initialize*app* 函数初始化了所有会话变量，包括所有 LLM、嵌入模型以及其他从左侧侧边栏选择的选项。

在 _agentic_rag.p_y 中的打印语句通过将 *sys.stdout* 重定向到 *io.stringIO* 缓冲区来捕获。然后，使用 Streamlit 中的 _text*area* 组件在调试占位符中显示该缓冲区的内容。
...
![智能指南的 Streamlit 界面（图片由作者提供）](img/43bc6af6040824d53e26839e5d4129fb.png)

智能指南的 Streamlit 界面（图片由作者提供）

以下图像展示了选择“*简洁*”回答风格时由 *llama-3.3–70b-versatile* 生成的答案。查询路由器 (_route*question*) 调用检索器（向量搜索），评分函数找到所有相关的检索块。因此，通过 _route_after*grading* 节点，决定通过 *generate* 节点生成答案。

![由 llama-3.3–70b-versatile 生成的 '简洁' 答案（图片由作者提供）](img/f85439219533c700d8e35cb918755bce.png)

由 llama-3.3–70b-versatile 生成的“*简洁*”答案（图片由作者提供）

以下图像展示了使用“*解释性*”回答风格对同一问题的回答。根据 _rag*prompt* 指示，LLM 通过更多解释来详细阐述答案。
...
以下图像显示了路由器在向量搜索中未找到相关片段时触发的 _get_license*info*工具。
...
以下图像显示了当在向量搜索中未找到相关片段时，由 _route_after*grading*节点触发的网络搜索。
...
以下图像显示了在*Streamlit*应用程序中选择混合搜索选项时生成的响应。_route*qustion*节点找到 _internet_search*enabled*状态标志“*True*”，并将问题路由到 _hybrid*search*节点。

![gpt-4 的混合搜索的“简洁”响应（图片由作者提供）](img/0776044167f0b52160a3fb662eef588d.png)

gpt-4 的混合搜索的“简洁”响应（图片由作者提供）
...
+   添加更多用于头脑风暴、写作和创意生成的代理。

* * *

*这就是全部了！* *如果你喜欢这篇文章，请为文章鼓掌（多次* 👏 *），写评论，并关注我的[Medium](https://medium.com/@umairali.khan)和[LinkedIn](http://www.linkedin.com/in/uakhan80)。*
