...
![图片由作者提供。](img/555478cccddfbfdfee1f5a81d617969f.png)

图片由作者提供。

***(如果您还没有阅读第一部分，请在此处查看[这里](https://medium.com/towards-data-science/missing-data-in-time-series-machine-learning-techniques-6b2273ff8b45)。)***
...
我的名字是[Sara Nóbrega](https://linktr.ee/saranobrega)，我是一名**[数据科学家](https://linktr.ee/saranobrega)**，专注于 AI 工程。我拥有物理学硕士学位，后来[转型](https://towardsdatascience.com/how-to-transition-from-physics-to-data-science-a-comprehensive-guide-ff1951090f65)到了令人兴奋的数据科学领域。

我撰写有关数据科学、人工智能以及数据科学职业建议的文章。请确保**[关注我](https://medium.com/@saranobregafn)并[订阅](https://medium.com/@saranobregafn/subscribe)**，以便在发布下一篇文章时收到更新！
...
1.  **为什么和何时使用非线性机器学习进行插补？**

1.  **第二部分：KNN 用于时间序列插补** 3.1 统计比较 3.1.1 自相关、STL 分解和残差分析
...
在**第一部分**中，我们使用了一个从 2023 年 1 月 1 日到 2023 年 3 月 1 日的 10 分钟间隔的模拟能源生产数据集。大约 10%的数据点为缺失值，以模拟插补的现实场景。
...
我们将使用来自**`scikit-learn`**的**`KNNImputer`**根据最近邻的已知观察值来填充缺失值。以下是如何操作的示例：
...
![图 2：比较线图，显示红色为原始能源生产数据（含缺失值），紫色为 KNN 插补数据。此图突出了插补值与 2023 年 1 月数据集的无缝集成 | 图片由作者提供。](img/54a075a3fafbc56a3e564c52dbe787fa.png)
...
**周期性编码**是一种转换，它将任何与时间相关的特征（如一天中的小时或一周中的某一天）转换为以这种方式表示该特征，从而捕捉这些变量中的自然周期。
...
![图 3：统计比较| 作者图片](img/c99981b8e1b7968597aa27f87c5c70be.png)
...
我们将创建**自相关函数（ACF）图并执行 STL 分解**，就像第一部分一样，以评估时间序列数据中的潜在**季节性模式、趋势和自相关**，这为插补结果提供了更深入的理解。
...
![图 4：比较原始（插值）和 KNN 插补数据集的时间依赖性的 ACF 图。两个图表都显示出相似的周期性模式，验证了时间依赖性的保持。| 作者图片](img/216a8c920f850d028c35d6f9269b62fc.png)

图 4：比较原始（插值）和 KNN 插补数据集的时间依赖性的 ACF 图。两个图表都显示出相似的周期性模式，验证了时间依赖性的保持。| 作者图片

图 4 显示了原始和插补数据集的[ACF 图](https://www.geeksforgeeks.org/autocorrelation-and-partial-autocorrelation/)。我们可以看到，两个图表都显示出非常相似的图案，证实了插补确实保留了时间依赖性！

从这些图中我们可以得出结论，插补后的数据集很好地捕捉了原始数据的周期性。

![图 5：比较原始（插值）和 KNN 插补数据集的 STL 分解趋势的趋势分析图。图表显示最小差异，表明插补数据保持了长期趋势。| 作者图片](img/d57b14bbd7739208a4612dd74399d6a5.png)

图 5：比较原始（插值）和 KNN 插补数据集的 STL 分解趋势分析图。图表显示最小差异，表明插补数据保持了长期趋势。 | 作者图片

![图 6：比较原始和插补数据的季节成分图 | 作者图片](img/132170683a024039ab0d0a85e777ca26.png)

图 6：比较原始和插补数据的季节成分图 | 作者图片

+   **图 5：STL 分解从 KNN 插补数据中提取的趋势成分**与原始数据非常接近，**反映出几乎没有偏差**。这证实了在插补过程中很好地保留了能源生产的长期模式！

+   **图 6**：同样，我们甚至无法从视觉上看到原始和插补数据集之间在**季节成分**上的任何差异。这表明能源生产周期中固有的日和周周期性得到了很好的保留。**实际上，从时间序列分析和预测任务的角度来看，这一点非常重要，尤其是在需要进一步下游工作的情况下。**

### 残差分析

![图 7：展示原始（插值）和 KNN 插补数据集的季节成分和残差的对比可视化。季节模式几乎相同，而残差在两个数据集中都表现出一致的噪声水平 | 作者图片](img/561b3bd98972d50639e374ab09e5e3a7.png)
...
1.  **可能变得计算复杂**：KNN 需要计算所有观察值的距离，如果你正在处理大型数据集或高维特征空间，这当然会变得计算昂贵。

1.  **对参数敏感**：`n_neighbors`的选择和加权方案会显著影响结果。

1.  **假设相似性：** KNN 假设可以通过在特征空间中接近其他观测值来推断缺失值。当数据中相似性太低，或者太嘈杂时，这可能会失败。

1.  **平稳性假设：** **KNN 本身不支持非平稳数据** – 具有变化趋势或季节性的时间序列数据 – 除了可能需要额外的预处理。我们这里使用的数据是平稳的。

1.  **可扩展性：** KNN 对于非常大的数据集或时间序列流数据扩展性不好。
...
### 如果您觉得这篇文章有价值，我会很感激您通过点赞来支持我。您也可以在这里[Medium](https://medium.com/@saranobregafn)关注我，阅读类似的文章！

**与我预约通话，向我提问或在这里发送您的简历：**

+   **[一对一辅导和简历审查](https://topmate.io/sara_nobrega)**
