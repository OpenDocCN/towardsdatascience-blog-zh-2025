...
> 原文：[`towardsdatascience.com/your-next-large-language-model-might-not-be-large-afterall-2/`](https://towardsdatascience.com/your-next-large-language-model-might-not-be-large-afterall-2/)

## <mdspan datatext="el1763754449550" class="mdspan-comment">引言</mdspan>

<mdspan datatext="el1768890078904" class="mdspan-comment">自 AI 概念提出以来</mdspan>，研究人员一直坚信规模的力量——普遍智能是随着规模的增长而出现的属性。如果我们只是继续增加参数并在庞大的语料库上训练它们，类似人类的推理就会显现出来。

但我们很快发现，即使是这种蛮力方法也有其自身的缺陷。证据表明，我们的大多数前沿模型都严重欠训练，参数计数过高[(Hoffmann 等人，2022)](https://arxiv.org/abs/2203.15556)³，这表明我们可能最终还是把计算资源用在了错误的方向上。
...
最后，也许是最关键的是，这些模型局限于一种“一刀切”的思考方法。例如，在处理一个非常困难的问题时，模型不能选择在问题的特定困难区域上花费额外的处理时间。当然，如果模型在更困难的问题上花费更多时间，它会产生更多的 CoT 标记([Wei 等人，2022](https://arxiv.org/abs/2201.11903))⁴。但这并不一定复制人类的推理，人类的推理涉及没有具体口头对话的深思熟虑的阶段。

## 分层推理模型

介绍分层推理模型（HRMs）([Wang 等人，2025b](https://arxiv.org/abs/2506.21734))¹：它们不是笨拙的“大声思考”方法，而是在它们本地的潜在空间中无声且流畅地推理——一个充满数字的丰富、高维世界。这更接近我们自己的直觉，深层次的思考往往在我们用言语描述它们之前。

这个新架构的核心是美丽而简单且动态的：一个耐心的高级 H 模块，负责设定整体策略，而一个快速的低级 L 模块则负责执行设定的策略。这两个模块都作为简单的转换器块[(Vaswani et al., 2017)](https://arxiv.org/abs/1706.03762)²堆叠在一起。

### HRM 是如何思考的：内部一瞥

它将“思考”的行为分解为一个动态的双速系统。为了理解它是如何解决像 30×30 迷宫这样的复杂问题，让我们从输入到答案的整个旅程进行回顾。

![](img/2f7772fb401bd9eec59e879ba4a3ec30.png)
...
在其核心，HRM 是一个嵌套循环，通过它的一次遍历被称为“段”。每个段包含几个*H*和*L*模块周期。
...
通过引擎（一个“段落”）的单次遍历可能不足以解决更微妙或更难的问题。这就是 HRM 最巧妙的功能所在：**自适应计算时间（ACT）** [(Graves, 2016)](https://arxiv.org/abs/1603.08983)⁶。
...
![](img/9a5d6bbea6592ec2a98a802239d6a7e8.png)
...
![](img/e29ed4a7b41c36373bc4e6be3dffbac3.png)
...
![图片](img/0aec4b31a1f586e534abf161773a2f51.png)
...
![图片](img/c1b0bbb42ca9390d6093798d9816361a.png)

（来源：改编自 [Wang et al., 2025](https://arxiv.org/abs/2506.21734)¹，图 1）
...
抽象和推理语料库（ARC）[(Chollet, 2019)](https://arxiv.org/abs/1911.01547)⁵是流体智能的广泛接受的标准，要求模型在只有少量视觉示例的情况下推断模糊和抽象的规则。HRM，仅 27 百万参数，就优于大多数主流推理模型。尽管其规模较小，但在 ARC-AGI-1 上得分 40.3%，而那些规模更大、计算能力强大的模型，如 o3-mini 和 Claude 3.7，分别得到了 34.5%和 21.2%的次优分数。

![图片](img/b1dede40e27d08c6ed35afc03060ce5b.png)
...
![图片](img/bbae9338d16f15213140c23ec8554a0e.png)
...
![图片](img/6825801c36fd2ee408637e0170e64c4b.png)
...
> **如果您喜欢这类分解，我在这里分享更多见解、笔记和解释：[`steadysurfdom.substack.com/`](https://steadysurfdom.substack.com/)**
...
[3] Hoffmann, Jordan, et al. “[训练计算最优的大型语言模型。](https://arxiv.org/abs/2203.15556)” *arXiv 预印本 arXiv:2203.15556* (2022).

[4] Wei, Jason, et al. “[思维链式提示引发大型语言模型的推理。](https://arxiv.org/abs/2201.11903)” *神经信息处理系统进展* 35 (2022): 24824-24837.

[5] Chollet, François. “[关于智能的度量。](https://arxiv.org/abs/1911.01547)” *arXiv 预印本 arXiv:1911.01547* (2019).

[6] Graves, Alex. “[循环神经网络的自适应计算时间。](https://arxiv.org/abs/1603.08983)” *arXiv 预印本 arXiv:1603.08983* (2016).
