...
> 原文：[`towardsdatascience.com/how-deep-feature-embeddings-and-euclidean-similarity-power-automatic-plant-leaf-recognition/`](https://towardsdatascience.com/how-deep-feature-embeddings-and-euclidean-similarity-power-automatic-plant-leaf-recognition/)
...
1.  创建一个在 UCI 一百种植物物种叶片数据集上完全可重复的流程，包括代码、评估以及结果的可视化。
...
我们的识别系统依赖于存储在 UCI 机器学习存储库（CC BY 4.0 许可）上的[一百种植物叶片数据集](https://archive.ics.uci.edu/dataset/241/one+hundred+plant+species+leaves+data+set)。这是一个包含 1,600 张高分辨率照片的集合，每张照片都有 100 种样本中的 16 个样本。这些物种包括常见的树木如橡树以及更多异国物种，它们在叶片形态学方面提供了丰富的多样性。
...
![图片](img/daf0762fe2a8dfcb6e1b9dcc63b035dc.png)

*照片由[Cope 等人（未注明日期）](https://archive.ics.uci.edu/dataset/241/one+hundred+plant+species+leaves+data+set)在[UC Irvine 机器学习存储库](https://archive.ics.uci.edu/dataset/241/one+hundred+plant+species+leaves+data+set)上提供*
...
![](img/2e6a45b6b8057323ce105ed2d2b24cb9.png)

由于所有嵌入都被归一化到单位长度，因此这个距离与它们的角区分度成正比，即：

![](img/28e216fdd585cb89e515acf372beb3d1.png)

其中 cos𝜃=𝑥⋅𝑦。较小的欧几里得距离意味着两个嵌入在特征空间中更相似，这增加了叶子是同一类的概率。

![](img/dcd4b74eefcecb64b82dc2cd8b3a464b.png)
...
![图片](img/471a87fc53bc636ca4d2ef84bc10c412.png)
...
![图片](img/80d8ce588d8bc2bb82401fed5ee6598d.png)

图片由作者提供

### 阈值调整的 ROC 曲线

为了以系统的方式推导出真实和假阳性之间的最佳决策边界，我们绘制了接收者操作特征（ROC）曲线，该曲线展示了在不同阈值下真阳性率（TPR）和假阳性率（FPR）之间的权衡。曲线上升表示对相同物种和不同物种对的判断得到改善。曲线下面积（AUC）是总性能的衡量标准，我们的系统具有出色的 AUC 值为 0.987，这确保了它在基于相似性的识别方面非常可靠。[Youden J](https://medium.com/data-science/optimal-threshold-for-imbalanced-classification-5884e870c293) 统计量最大化了最佳阈值的敏感性和特异性（0.68）。

![图片](img/83164ac7d37f500d552fe9b3ef31d4d6.png)
...
![图片](img/6de260edfac0a484b13dd2fe716ba7e1.png)
