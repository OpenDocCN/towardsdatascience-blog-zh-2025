...
> 原文：[`towardsdatascience.com/the-absolute-beginners-guide-to-pandas-dataframes/`](https://towardsdatascience.com/the-absolute-beginners-guide-to-pandas-dataframes/)
...
现在我们有了我们的数组。让我们将其传递到我们的 DataFrame 中。要创建 DataFrame，请使用`pd.DataFrame()`函数。
...
这可能是你将最常使用的方法。在尝试进行数据分析时，在 pandas 中读取 CSV 文件是一种常见的做法。类似于在 Excel 中打开电子表格或导入数据到 SQL。在 Python 中，你通过使用`read_csv()`函数来读取 CSV 文件。以下是一个示例：
