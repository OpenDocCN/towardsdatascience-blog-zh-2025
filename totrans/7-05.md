...
> 原文：[`towardsdatascience.com/learn-to-build-agentic-ai-systems-9e552d841525/`](https://towardsdatascience.com/learn-to-build-agentic-ai-systems-9e552d841525/)

![图片由 Photo By: Kaboompics.com 来自 Pexels.](img/cea7d1faf3a16b320f9dc8cab8373d64.png)

图片由 Photo By: [Kaboompics.com](https://www.pexels.com/photo/young-woman-with-laptop-sitting-on-yoga-mat-4498457/)来自[Pexels](https://www.pexels.com/photo/young-woman-with-laptop-sitting-on-yoga-mat-4498457/)提供。

> 当红话题：AI 代理将取代 SAAS。

这并不是任何毒品贩子随口说出的一句话。这是微软首席执行官[萨蒂亚·纳德拉](https://economictimes.indiatimes.com/tech/artificial-intelligence/ai-agents-will-revolutionise-saas-and-productivity-microsoft-ceo-satya-nadella/articleshow/117026938.cms?from=mdr)说的。

如果你是一名开发者或技术领域的任何人，你都会想知道如何自己构建 AI 代理。但并不是我们中的很多人都能抽出时间从头开始学习它们。

如果你发现自己属于这个群体，我写这篇文章就是为了你。

这也是我学习如何开发 AI 应用的方式——但更加非正式。因此，我想在文章中组织它，这样你可以很好地跟随，并用你的周末时间学习开发 AI 应用的基础。
...
你需要一个活跃的互联网连接，因为我们将通过它们的 API 使用 OpenAI 模型。哦，这也意味着你需要一个活跃的[OpenAI API 密钥](https://platform.openai.com/)。

你的 PC 应该与 Chroma 等向量存储兼容，并且能够运行它们而不会出现问题。
...
此外，在你的工作目录中创建一个`.env`文件，并用你的 OpenAI API 密钥更新它。
...
这是我在学习 LLMs 时创建的最简单、最有趣的工具。

我并没有直接使用这个应用程序，但它让我对 LLMs、提示和 Langchain 生态系统有了坚实的理解。

这个应用程序将研究论文总结成几行易于消化的内容。
...
这是一个超级简单的应用程序。然而，它让我们对 LangChain 生态系统以及如何用它来构建 LLM 驱动的应用程序有了更深的了解。

在这个应用程序中，我们读取网页的内容（确切地说是一篇关于 G-Eval 的研究论文）并使用 GPT-4o-mini 进行总结。为了使这个模型工作，我们应该将 OpenAI API 密钥设置为环境变量，我们在上一节中已经这样做过了。

我们应该注意提示模板和链。提示模板是一个带有任何我们动态插入的变量的占位符的提示。大括号内的任何内容都会被视为变量名。我们必须在运行时设置这个，以从提示模板创建提示。

在我们的案例中，我们将变量命名为`{context}`。

接下来，我们创建了一个名为链的东西。链就像一个函数，它将执行管道的各种组件粘合在一起。输入变量（`context`）、提示、llm 以及更多将在稍后介绍。我们现在可以使用输入变量调用（或者更准确地说，调用）链。

### I. 回答问题（不仅仅是总结）

最后一个例子只使用了单个输入变量。然而，让我们修改提示模板以接受两个变量。

这次，我们传递了上下文和一个问题。因此，用户不仅可以总结上下文，还可以就上下文提出任何随机的问题。
...
在新版本中，提示被简化为根据`context`回答用户的`question`。请注意，现在调用方法同时传递了作为字典的输入变量。

### II. 复杂的链

如果你仔细看，调用方法的输出是一个对象。如果我们想得到一个只包含文本的、其他应用程序可以使用的漂亮输出呢？

在以下例子中，我展示了如何使用 Langchain 的内置输出解析器格式化输出。我还运行了一个额外的 lambda 函数来展示我们可以在链中运行任何函数。
...
之前的应用程序很有帮助，但仅限于一定程度。

该应用程序的主要限制是输入的大小。它只是一个网页，而不是《哈利·波特》系列的集合。

最新模型支持大上下文窗口。例如，[Google 的 Gemini 1.5 可以处理高达 100 万个标记](https://blog.google/technology/ai/google-gemini-next-generation-model-february-2024/#sundar-note)。在英语中，这大约相当于 75 万个单词。

这样就足以舒适地拥有一个学术教科书。但这里还有一些其他问题需要考虑。

1.  在更大的上下文中，大型语言模型（LLM）往往会失去焦点。当上下文中存在过多的**噪声**时，LLM 很难找到我们具体问题的答案——结果往往是幻觉。

1.  你还需要为输入令牌付费。像 OpenAI 这样的 LLM API 提供商**按输入和输出令牌收费**。在更大的上下文中，你必然需要为这些提供商付费。更不用说**互联网带宽的成本**了。

1.  更大的上下文意味着**更慢的响应**。这不是火箭科学；你提供了太多信息供其他 LLM 遍历，你应该期待更慢的响应。如果你使用 API 提供商，你的响应将因互联网流量而进一步延迟。

由于以下原因，向 LLM 提供仅相关的信息是明智的。例如，如果用户问关于闪电的问题，而不是传递整本物理教科书作为上下文，过滤掉提到闪电（或相关术语）的部分，并提供这些部分。

这种方法被称为检索增强生成，或简称为**RAG**。

> [**如何使用查询路由构建有用的 RAG**](https://towardsdatascience.com/rags-with-query-routing-5552e4e41c54)

大多数 RAG 应用遵循以下方法。

1.  您的数据被分割成更小的段，以限制每个块中只有一个想法。

1.  每个块都被转换为向量嵌入并存储在向量存储中，如 Chroma。

1.  在检索过程中，用户的提问也被转换为向量嵌入。

1.  向量存储将计算问题的向量版本与数据库中块现有向量的相似度。

1.  在 RAG 链中，具有最高相似度的前 n 个块将成为回答你问题的**上下文**。
...
在上面的例子中，我们在 Chroma DB 中索引了一个网页（Django 官方文档页面之一）。

请注意第 2 步，我们分割和索引网页并存储在 Chroma DB 中。为了分割网页的文本内容，我们使用了递归字符分割技术，这是我们创建块的方式之一。

> [**高级递归和后续检索技术，以实现更好的 RAG**](https://towardsdatascience.com/advanced-retrieval-techniques-for-better-rags-c53e1b03c183)

我们随后使用 OpenAI 嵌入将块转换为它们的向量表示，并存储在 Chroma DB 中。需要注意的是，当我们为检索到的问题创建向量版本时，我们需要使用相同的嵌入模型。幸运的是，Langchain 已经处理了这一点。

现在注意第 4 步，我们创建 RAG 链。这可能与我们的上一个应用看起来相似，但有两大明显区别。在上下文的位置，我们现在传递检索器。检索器是一个准备为你获取信息的向量存储。接下来，我们不再使用问题，而是有一个 RunnablePassthrough 对象。这允许我们以问题作为唯一参数调用链。我们不再需要传递字典。
...
但在底层，每次你提问（每次你调用链）时，链都会将问题转换为向量，从向量存储中检索相似片段，将其作为上下文传递给我们的 LLM，并生成对我们问题的答案。

恭喜你构建了你的第一个 RAG 应用。我们现在准备好应对 AI 代理了。

## **#3 使用 AI 代理完成任务**

“AI 代理”这个术语可能会让你困惑，甚至让你感到害怕，但它是一个简单的概念。

将代理视为一个你分配任务的工人。然后工人必须使用工具来完成任务。

这听起来像是一个传统的软件程序。确实如此。然而，与需要按特定顺序执行指令的软件不同，代理将确定要使用哪些工具以及使用顺序。
...
![图片由作者提供。](img/714e8b76ecad660d27b71ce3cc86ed70.png)

图片由作者提供。

整个报告只用了 26 秒就完成了。这听起来可能有点高。但这里的关键点。

你手动完成它需要多长时间？即使你能编写代码来完成它，你完成那个 Python 脚本需要多长时间？

这段代码只有几行长。但它可以使用单个工具，YFinanceTool，完成无数的事情，该工具可以获取与股票相关的信息。请注意，我们并没有告诉 LLM 要查找哪些公司。LLM 自己解决了这个问题。
...
在这里，我们要求代理阅读一个链接并找到链接中提到的公司的最新新闻。链接是 Y-combinator 资助的 Gen-AI 公司列表。

我们预计第一个工具，Newspaper4k，将阅读文章，然后第二个工具，DuckDuckGo，为链接中提到的每个初创公司找到相关新闻。

这里是输出：

![](img/98f0420d592f64ca83dcbfc155c0a542.png)

这个任务手动执行或甚至编码都不容易，但我们的代理快速完成了它。

### II 多代理架构

有时候，让专门的代理一起工作来完成我们的任务是最好的。

与将所有工具都给一个人并要求他们执行复杂任务相比，你最好将其分配给更多专门的人。你的每个团队成员都将有专门的方式使用他们可用的工具来完成子任务。将有一个领导者来协调其他成员之间的事务。

这听起来很疯狂。

这就是多代理系统的样子。我们将要构建一个。
...
上述代码有两个代理。第一个代理专门在互联网上搜索新闻。它使用 Tavily 工具进行编程搜索。第二个代理从雅虎财经获取财务信息。

我们还有一个第三个代理。

这个任务只有一个职责——协调两个代理来完成它所要求的任务。

当我们要求代理团队“找到最新的科技新闻，然后获取并总结新闻中提到的公司的最新股票详情”时，输出的结果如下。

![作者截图](img/29e8af48a0f0c82f1d90bf76c5ffc93d.png)

作者截图。

研究代理已经搜索了像福布斯和 MSN 这样的可信来源，以找到最新的科技新闻。然后，它使用了 YFinanceTool 来获取最近的股票信息。随后，它很好地呈现了一份包含总结的报告。

## 最后的想法

多代理架构有很大的潜力自动化我们许多繁琐的任务。它们可以在代理之间进行协调，这是确定性编码所不可能做到的。

像 Langchain 这样的工具可以帮助任何了解 Python 的人开发由 LLMs 驱动的智能应用。同样，像 Phidata 和 Crew AI 这样的工具也可以帮助你构建代理和代理团队。

启动这些任务所需的时间也不是太疯狂。也许你只需要一个周末和几杯咖啡。
