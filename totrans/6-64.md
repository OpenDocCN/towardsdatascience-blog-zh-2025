...
当 SQL 聚合函数应用于大型数据集时，可能会非常耗费计算资源。随着数据集的增长，反复在整个数据集上重新计算指标变得效率低下。为了应对这一挑战，**增量聚合**通常被采用——这是一种涉及维护先前状态并使用新传入数据更新它的方法。虽然这种方法对于 COUNT 或 SUM 这样的聚合来说很简单，但对于标准差这样的更复杂指标，问题就出现了：如何将其应用呢？

[标准差](https://en.wikipedia.org/wiki/Standard_deviation)是一种统计指标，它衡量了变量值相对于其平均值的变异或分散程度。它是通过取方差的平方根得到的。[方差](https://en.wikipedia.org/wiki/Variance)的计算公式如下：

![样本方差公式](img/a740f71788ea7a1d8d6d0c0d0f50e3cb.png)

样本方差公式

计算标准差可能很复杂，因为它涉及到更新所有数据点的平均值和平方差的和。然而，通过代数变换，我们可以推导出一个增量计算的公式——允许使用现有数据集进行更新并无缝地结合新数据。这种方法避免了每次添加新数据时都需要从头开始重新计算，使整个过程更加高效（详细的推导可以在我的 GitHub 上找到：[https://github.com/YuvalGorch/math_proofs/blob/main/incremental_stddev_proof.pdf]）。

![导出的样本方差公式](img/4fca54cd3a23b2c9bb93c97533c98baa.png)
...
此方法通过保留现有集合的 COUNT (k)、AVG (µk) 和 VAR (Sk)，并将它们与新的集合的 COUNT (n)、AVG (µn) 和 VAR (Sn) 结合起来，实现了增量方差计算。因此，可以高效地计算更新后的标准差，而无需重新扫描整个数据集。

现在我们已经理解了增量标准差背后的数学原理（至少抓住了其精髓），让我们深入探讨 dbt SQL 的实现。在下面的示例中，我们将介绍如何设置一个增量模型来计算和更新用户的交易数据中的这些统计数据。

考虑一个名为**stg__transactions**的交易表，它跟踪用户交易（事件）。我们的目标是创建一个时间静态表，**int__user_tx_state**，该表聚合用户交易的状态。两个表的列详细信息如下图中所示。

![作者图片](img/56200361ee96923ee490c50dd75af9e8.png)

作者图片

为了使过程高效，我们旨在通过将新到达的交易数据与现有的聚合数据（即当前用户状态）结合起来，增量地更新状态表。这种方法允许我们计算更新的用户状态，而无需扫描所有历史数据。

![作者图片](img/16bce03be1aff0e1acb238599dabe6e5.png)
...
我们将逐步构建一个完整的 dbt SQL 步骤，目的是高效地计算增量聚合，而不需要重复扫描整个表。这个过程从在 dbt 中将模型定义为增量开始，并使用`unique_key`来更新现有行而不是插入新行。
...
接下来，我们从**stg__transactions**表中获取记录。`is_incremental`块通过时间戳过滤出晚于最新用户更新的交易，有效地包括“仅新交易”。
...
在检索到新的交易记录后，我们按用户对它们进行聚合，这使得我们可以在以下 CTE 中逐步更新每个用户的状态。
...
现在我们来到了重头戏，我们需要实际计算聚合。当我们不在增量模式（即我们还没有任何“状态”行）时，我们只需简单地选择新的聚合。
...
但当我们处于增量模式时，我们需要根据上述公式将过去的数据与我们在`INCREMENTAL_USER_TX_DATA` CTE 中创建的新数据结合起来。我们首先计算新的 SUM、COUNT 和 AVG：
...
1.  如前所述的平均差异方差，以及 SQL 连接项以包括过去的数据。
...
在整个过程中，我们展示了如何有效地处理非增量模式和增量模式，利用数学技术高效地更新像方差和标准差这样的指标。通过无缝结合历史数据和新的数据，我们实现了一种优化、可扩展的实时数据聚合方法。
