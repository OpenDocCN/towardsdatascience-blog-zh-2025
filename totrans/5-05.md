# 商业智能中的主题建模：Code 中的 FASTopic 和 BERTopic
...
**客户评价**关于产品和服务的提供有关客户满意度的宝贵信息。它们提供了关于整个产品开发中应该改进什么的见解。商业智能中的动态主题模型可以识别关键产品品质和其他满意度因素，将它们聚类到类别中，并评估业务决策如何随着时间的推移在客户满意度中体现。这对产品经理来说是非常有价值的信息。

本文将比较两种最新的主题模型来分类客户投诉数据。由[Maarten Grootendorst (2022)](https://arxiv.org/pdf/2203.05794)提出的**BERTopic**和去年[NeurIPS](https://neurips.cc/virtual/2024/poster/96416)上提出的由[Xiaobao Wu 等人 (2024)](https://arxiv.org/pdf/2405.17978)提出的最近的**[FASTopic](https://pypi.org/project/fastopic/)**，是目前客户数据主题分析的前沿模型。对于这些模型，我们将用 Python 代码进行探索：

+   如何有效地**预处理数据**

+   如何训练一个**二元主题模型**进行客户投诉分析

+   如何随时间**建模主题活动**

# 1. 公司中的客户投诉数据

投诉数据是通过与客户的互动生成的，通常记录在[ERP 系统](https://www.sap.com/products/erp/what-is-erp.html)中。客户可以提出关于产品或服务的担忧的渠道有很多，以下只是其中的一些例子：

+   **电子邮件**：电子邮件通信被存储在 BI 团队中，例如，在 SQL 数据库中。

+   **售后服务****调查**：在产品购买后发送给客户的反馈。公司要么自己发送电子邮件，要么使用价格比较网站（例如，德国的[Billiger](https://www.billiger-mietwagen.de/)），客户在那里订购产品。
...
+   **评论平台**：独立评论平台 **** (例如 [Trustpilot](https://www.trustpilot.com/)) 为客户提供向品牌和公司提供反馈的地方。这些数据可以通过 [各种 API](https://developers.trustpilot.com/service-reviews-api) 获取。

+   **社交媒体对话**：Instagram、X 和 Facebook 上充满了产品或品牌相关的评论。最简单的方法是使用官方 API 收集数据。对于 Instagram 和 Facebook，请访问 [开发者门户](https://developers.facebook.com/) 获取 API 密钥。X 以相同的方式工作。[点击这里](https://developer.x.com/en/docs/x-api)。

## 2. 示例数据

作为示例数据，我们将使用来自 **Hugging Face** 的 **[亚马逊狗粮评论](https://huggingface.co/datasets/NiyatiC/amazon_food_reviews)** 数据集，该数据集在 [Apache-2.0 许可证](https://github.com/huggingface/datasets/blob/main/LICENSE)下发布。用于主题建模的子集仅包含 3693 条在 02/01/2016 至 31/12/2020 期间收集的客户评论。以下是数据的样子：

![图像 1. 亚马逊狗粮评论数据集](img/97ed53fa122597a1c65a97f8d20c2b26.png)

图像 1. 亚马逊狗粮评论数据集

![图像 2. (客户反馈)主题建模的一般预处理步骤。图由作者提供](img/cb486603b34c90392dab5036a8ff05c5.png)

图像 2. (客户反馈)主题建模的一般预处理步骤。图由作者提供

## 3. 数据预处理

按正确顺序系统地处理数据，可以保留关键信息，不会添加新的偏差。让我们按照以下步骤进行：
...
+   ***#4: 标点符号：*** 通用语言中有无数的特殊字符和标点符号，在这一步应该进行清理。

+   ***#5: 额外停用词：*** 根据用例，一些额外的单词也有助于去除。在亚马逊狗粮评论中，这些是 *"dog"，"food"，"blue"，"buffalo"，"ha"，"month"* 和 *"ago"。*

> "Delivery" 和 "deliveries"，"box" 和 "Boxes"，或 "Price" 和 "prices" 具有相同的词根，但如果没有词形还原，主题模型会将它们建模为不同的因素。这就是为什么产品评论应该在预处理步骤的最后一步进行词形还原。

+   ***#6: 词形还原：*** 将单词归入单一形式（词元），保留词根信息和语义。

文本预处理是针对特定模型的：

+   ***FASTopic*** 在输入数据上使用清洁数据；一些清洁（停用词）可以在训练期间完成。最简单、最有效的方法是使用 ***[Washer: 无代码文本数据清洗应用](https://washer.textminingstories.com/)***，它提供了一种无代码处理数据用于文本挖掘项目的方法。

+   ***BERTopic:*** 该[文档](https://maartengr.github.io/BERTopic/faq.html#how-do-i-reduce-topic-outliers)建议，“作为预处理步骤，不建议移除停用词，因为我们使用的基于 transformer 的嵌入模型需要完整的上下文来创建准确的嵌入”。它使用基于真实文本的 transformer，而不是没有停用词、词元或标记的清洁文本。因此，清洁操作应包含在模型训练中。

![来源：Freepic，图由 macrovector 提供](img/0644d607a8c089a179083188347f8479.png)

来源：[Freepic](https://www.freepik.com/free-vector/romantic-train-trip-background_5972223.htm#fromView=search&page=1&position=24&uuid=1b2ccb61-1b56-453e-b67a-befe1eba6858)，图由 macrovector 提供
...
[BERTopi](https://arxiv.org/pdf/2203.05794)c 和 [FASTopic](https://arxiv.org/pdf/2405.17978) 论文详细描述了模型架构。此外，我的 [TDS 教程](https://towardsdatascience.com/topic-modelling-with-berttopic-in-python-8a80d529de34) 在政治演讲数据集上解释了使用 BERTopic 进行主题分类。

### 4.1. FASTopic

导入库和数据（完整的代码和需求[在此](https://github.com/PetrKorab/Topic-Modelling-in-Business-Intelligence-BERTopic-and-FASTopic-in-Code/tree/main)）。然后，创建一个清洁评论列表：
...
在 FASTopic 中，二元组生成不是直接实现的。为了解决这个问题，我们将创建一个二元组预处理类。该模型以与单个标记相同的方式处理二元组，因此我们使用下划线连接二元组中的单词。
...
让我们训练一个包含八个主题的模型，并在数据框中显示每个主题的前 20 个二元组。我们首先在单个标记上训练，然后删除下划线以生成二元组。
...
我们使用狗粮产品对客户满意度因素进行了八个不同的主题建模。以下是手动标注的主题名称：

![Image 3: 使用 FASTopic 进行满意度因素建模。图由作者提供](img/44489c5d9c85709ae603fbc37a6ff234.png)

Image 3: 使用 FASTopic 进行满意度因素建模。图由作者提供

FASTopic 返回相对独特的主题，对客户的评论进行排序：

+   **0: 健康影响，** *"敏感的胃", "小咬", "胃部问题", "减肥", "拒绝进食", "味道怪异", "消化问题", "胃部不适", "停止进食", "增重"*

+   **1: 食品质量，** "喜欢味道", "优质成分", "好成分", "健康成分", "成分质量", "味道好", "味道极佳", "健康之爱", "极健康", "好健康", "好健康", …*

+   **2: 正面健康效果，** "健康皮毛", "了不起的幼犬", "眼睛明亮"*

+   **3: 消化效果，** "气味难闻", "腹泻", "可怕的气体", "腹泻兽医", "可怕的腹泻", "生病的一周", "生病购买", "一天呕吐"

+   **4: 定价，** "好价格", "好价格", "爱价格", "价格好", "爱便宜", "价格配送", "大优惠", "价格上涨", "免费送货", …*

+   **5: 其他，** 其他因素。

+   **6: 毛发效果，** "皮毛闪亮", "毛孩", "皮肤问题", "闪亮的皮毛", "爱毛", "皮毛柔软"*

+   **7: 配送，** "打开的箱子", "袋子撕裂", "大袋子", "洞袋", "打开的袋子", "箱子内部", "袋子打开", "袋子洞", "重的袋子", "撕裂打开", …*

检查这些类别在数据中的权重也是有用的。完整代码[在此](https://github.com/PetrKorab/Topic-Modelling-in-Business-Intelligence-BERTopic-and-FASTopic-in-Code/blob/main/FASTopic.ipynb)。

> 我们已经使用狗粮产品建模了客户满意度因素。但这对公司有什么好处？动态主题模型提供了一种简单直接的方法来监控客户满意度随时间的变化。它们表明与产品相关的问题，并有助于采取正确的措施。一旦业务决策付诸实施，主题模型将检查它们是否随时间产生影响。

为了做到这一点，让我们以季度频率建模主题活动随时间的变化。
...
主题 7 中的配送问题在 2018 年第三季度达到顶峰。客户经常抱怨打开的和撕裂的箱子，但这些问题在 2019 年初得到了解决（见下图）。

![图像 4：主题活动随时间变化，FASTopic。图由作者提供](img/f7a81deed37be4d9e9cf3ed763af3db5.png)

图像 4：主题活动随时间变化，FASTopic。图由作者提供。

### 4.2\. BERTopic

BERTopic 使用 _[vectorizer_model](https://scikit-learn.org/1.5/modules/generated/sklearn.feature_extraction.text.CountVectorizer.html)_ 实现了二元组，它也作为一个数据处理管道。代码和需求[在此](https://github.com/PetrKorab/Topic-Modelling-in-Business-Intelligence-BERTopic-and-FASTopic-in-Code/tree/main)。
...
我们在原始数据上训练并使用向量器进行清理。在训练过程中，向量器从数据中清理数字和停用词，为二元组模型返回词元化标记。
...
注释的主题显示与 FASTopic 类似的分类。差异在于将西班牙语标记分类到单独的主题（T7）以及用积极意义的形容词填充 T1 和 T5。T4 中的配送问题与 FASTopic 的分类相同。

![图像 5：使用 BERTopic 进行满意度因素建模。图由作者提供](img/62616eab4bdbbf16cfa9bc2559f42fb2.png)

图像 5：使用 BERTopic 进行满意度因素建模。图由作者提供

再次，让我们关注随时间变化的主题活动，这为动态主题模型在商业智能方面提供了额外的价值。BERTopic 使用标记频率（而不是主题权重，如 FASTopic）进行主题活动分析。
...
大多数主题在时间上都是稳定的，除了 T4，它将配送问题分类。与 FASTopic 类似，BERTopic 显示客户对损坏箱子的负面投诉在 2018 年中增加。
...
两个模型都指出在 2018 年中存在问题，这些问题在 2019 年初消失。通过使用主题模型 API 监控各种渠道的客户评论，这些问题可以在它们对品牌造成有害影响之前得到解决。

**正确的数据处理**对于主题模型在应用世界中具有意义至关重要。以正确的顺序清理文本可以最小化每次清理操作的偏差。通常首先删除数字和表情符号，然后是停用词。之后清理标点符号，这样停用词就不会被分成两个标记（"we’ve" -> "we" + ‘ve'）。在词形还原之前，在清理数据中删除额外的标记，以统一具有相同语义的标记。

**FASTopic** 值得更好的[文档](https://github.com/BobXWu/FASTopic)，目前它只提供基本信息。特别是由于其（1）使用简单和（2）在小数据集上训练的稳定性，使其成为 BERTopic 的顶级替代品。它主要适用于像网店这样的小型公司，这些公司通常不收集大量文本数据集，并寻求简单高效解决方案。本教程的数据和完整代码[在此](https://github.com/PetrKorab/Topic-Modelling-in-Business-Intelligence-BERTopic-and-FASTopic-in-Code/tree/main)。

*如果您喜欢我的工作，您可以邀请我[喝咖啡](https://www.buymeacoffee.com/petrkorab)并支持我的写作。您还可以订阅我的[电子邮件列表](https://medium.com/subscribe/@petrkorab)，以获取关于我新文章的通知。谢谢！*
