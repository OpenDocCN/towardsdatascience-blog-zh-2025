...
数据科学家头衔不断变化且往往模糊不清。它通常涉及一个精通数学、编程和机器学习的人。他们花费时间清理数据、构建模型、微调和进行实验。他们还必须具备出色的沟通技巧、对其领域的良好把握以及其他软技能。

然而，情况并不总是如此。如果你花足够的时间浏览职位列表，“数据科学家”这个职位可能会有很大的不同。有些更像数据工程师，专注于管道和大数据平台。有些更接近数据分析师，专注于数据清理和仪表板。而最近，许多人更像软件或机器学习工程师，专注于面向对象编程、构建应用程序、部署模型，有时甚至涉及网页开发。

![图片由作者提供](img/2768bb86f3175801b0e56e30ef030390.png)

图片由作者提供

有些人期望这一切以及更多，因此，出现了“全栈数据科学家”。考虑到这一点，数据科学家应该考虑超越在笔记本中开发模型，并将他们的技能扩展到其他领域，如机器学习运维。正如[Pau Labarta Bajo](https://datamachines.xyz/)所说：“**Jupyter 笔记本中的机器学习模型具有 10.0 的商业价值**。”

**本文将介绍数据科学家如何通过使用 FastAPI 和 Docker，将他们的机器学习模型从笔记本成功部署到完全生产化的 API。**

### 对“全栈”数据科学家的思考

首先，我对“全栈数据科学家”的个人看法。随着所有这些新兴期望的出现，对我们来说，学习和熟悉我们可能在教育或早期职业生涯中未学到的其他技能是很重要的。然而，期望似乎是掌握所有这些技能，同时还要跟上传统的数据科学。虽然有些人能够做到这一点，但这对我们大多数人来说是不切实际的。

我认为成为一名全栈数据科学家并不意味着掌握所有这些技能、技术等。**我认为，全栈数据科学家是通过持续学习和发展的能力，能够佩戴数据科学生命周期中的所有帽子。**

虽然这可能不是我的专长，但我应该能够与数据工程师合作以优化管道。而且，尽管我在开发模型方面感到更加自在，但我应该能够戴上我的“机器学习工程师”帽子，并帮助将模型部署到生产环境中。一位优秀的数据科学家总会拥有自己的领域，但也会对其他领域有实际的知识，并且能够在需要时快速学习新技能。

* * *

## 模型开发

首先，在我们的例子中，我们需要开发一个模型。**由于这篇文章侧重于模型部署，我们不会担心模型的表现。** **相反，我们将构建一个具有有限特征的简单模型，以专注于学习模型部署。**

在这个例子中，我们将根据一些特征（如经验、职位、公司规模等）预测数据专业人士的薪水。

*见数据于此：[`www.kaggle.com/datasets/ruchi798/data-science-job-salaries`](https://www.kaggle.com/datasets/ruchi798/data-science-job-salaries) (CC0: 公共领域)。我稍微修改了数据，以减少某些特征的选项数量。*
...
![Image by Author](img/02eaa9cf57373dec5358722b42a3f026.png)

Image by Author

由于我们的所有特征都是分类的，我们将使用编码将我们的数据转换为数值。下面，我们使用顺序编码器来编码经验水平和公司规模。这些是顺序的，因为它们代表某种进步（1 = 入门级，2 = 中级，等等）。
...
![Image by Author](img/f54cca73a46fa44c8e0154a5e1f21bfa.png)

Image by Author

看起来我们的 R 平方是 0.27，哎呀。这个模型还需要做更多的工作。我们可能需要更多的数据和关于观察结果的额外信息。但为了这篇文章，我们将继续前进并保存我们的模型。
...
部署模型有几种方法。其中一种方法是通过 API。**API（应用程序编程接口）使两块软件能够相互通信。** 有几种 API 架构，如 SOAP、RPC 和 REST API。我们将使用 REST API，这是最受欢迎和最灵活的架构，用于访问服务。

对于我们的框架，我们将使用 FastAPI ([`fastapi.tiangolo.com/`](https://fastapi.tiangolo.com/))，它非常适合初学者，因为它相当容易使用，并且有大量的文档和示例。
...
现在，让我们使用命令行来测试 API。首先，将目录更改为您的项目。然后，使用 uvicorn 运行 API。
...
命令行给了我一个链接。然后我被 GET 端点的消息欢迎。太好了！

![图片由作者提供](img/013b8a7c438d43351d4c0bb40bb736ef.png)

图片由作者提供

最后，让我们创建一个测试脚本，提交新的数据并检索预测。使用 requests 库，我们定义 URL 并提交一个新的观测值。
...
![图片由作者提供](img/11f958e21f025c7e7428b31749617452.png)

图片由作者提供

由于 POST 端点，预测以 JSON 格式返回。太好了，我们有一个运行的 API！
...
现在我们有了一种与我们的模型交互的方式，但模型尚未部署。假设我们有一个 20 人的团队，我们希望所有人在他们的电脑上运行 API。这可能会很头疼。复制数据科学应用可能具有挑战性，因为存在许多障碍，例如不同的操作系统、依赖项、技术堆栈等。

这就是 Docker 发挥作用的地方。Docker 是一个平台，它使开发者能够将他们的应用程序及其所有依赖项打包在"容器"中。任何可以访问容器的人都可以运行应用程序，而无需担心下载正确版本的包、更改操作系统等。Docker 容器也非常快速和轻量级，这比虚拟环境或机器具有优势。

在此处下载 Docker Desktop: [`www.docker.com/`](https://www.docker.com/)

### 创建 DockerFile 和镜像

在我们创建容器之前，我们必须首先创建一个镜像。Docker 镜像是应用程序及其依赖项的快照。它基本上概述了容器运行的指令。

要创建一个镜像，你必须创建一个 Dockerfile ([`docs.docker.com/reference/dockerfile/`](https://docs.docker.com/reference/dockerfile/))。Dockerfile 是一个基于文本的文档，存储在项目内部，提供了如何构建镜像的指令。Dockerfile 不能是.txt 文件。它必须没有扩展名。**创建 Dockerfile 最简单的方法是通过 VSCode。只需添加一个新文件，并将其命名为"Dockerfile"。**

我根据他们的入门级文档构建了以下 Dockerfile。它遵循以下步骤：
...
![图片由作者提供](img/cc885dff0bd8639203a4106bacace361.png)

图片由作者提供

### 创建 Docker 容器

现在我们有了镜像，创建容器非常简单。一旦我们运行了镜像并指定了一些指令，容器就创建完成了。下面，我们运行镜像并指定端口。
...
如果我们再次导航回 Docker Desktop，我们可以看到容器。Docker 为容器分配随机名称，这可能会变得难以追踪。如果你开发了许多应用程序，重命名它们是有用的。

![作者提供的图片](img/04c614032f8336fc0372892d8bd09f3e.png)

作者提供的图片

模型现在已部署！回到我们 20 人的团队，他们只需要在机器上安装 Docker 并访问我们的容器。然后他们就可以运行容器并按需使用 API。

* * *

## 结论

总结来说，随着对数据科学家的新期望，学习其他技能如软件工程和机器学习运维（ML Ops）变得至关重要。随着组织需要能够参与数据科学生命周期所有阶段的人才，对“全栈数据科学家”的需求正在增长。

将机器学习模型从笔记本中移出并投入生产是成为全栈数据科学家的重要第一步。通过使用 FastAPI 和 Docker 等工具，你可以通过允许他人也使用它来分享构建模型所付出的辛勤工作。
