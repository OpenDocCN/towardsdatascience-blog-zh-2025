...
> 原文：[`towardsdatascience.com/the-beauty-of-space-filling-curves-understanding-the-hilbert-curve/`](https://towardsdatascience.com/the-beauty-of-space-filling-curves-understanding-the-hilbert-curve/)

## 0. 简介

<mdspan datatext="el1757106707102" class="mdspan-comment">填充曲线</mdspan>（SFC）是迷人的数学结构，在数据科学和数据工程中有许多实际应用。虽然它们可能听起来很抽象，但它们通常隐藏在显而易见的地方——例如在 Z 排序或液态聚类（例如在 Databricks 等平台上使用）等术语背后。如果你已经与大规模数据平台合作过，那么你很可能已经在不经意间使用了 SFCs。
...
（如果你想直接从可视化动画开始，请查看我的[GitHub 仓库](https://github.com/PaulFroehling/Hilbert-Curve-Animation)）
...
![图片](img/174267d70abe0e1eb9b28237a6b38c07.png)
...
![图片](img/532c8468388fdde77381ebb4a3a4cc1b.png)
...
![图片](img/c47d2c8e0e360a5831efe77ae9f2bb47.png)
...
![图片](img/e0ba92feb9e4010a1880e53857762323.png)
...
+   **正 Jordan 内容**：在无限多次迭代的过程中，希尔伯特曲线的图像具有正 Jordan 测度，这意味着它占据了平面上的非零面积。（[佩亚诺-乔丹测度](https://en.wikipedia.org/wiki/Peano%E2%80%93Jordan_measure)）

+   **二维的豪斯多夫维度**：相应地，希尔伯特曲线不像通常的一维线那样表现，而是具有[Hausdorff 维度](https://de.wikipedia.org/wiki/Hausdorff-Dimension) 2，这反映了它完全填充了单位正方形。
...
在互联网上搜索可能的实现方法时，我偶然发现了一个普林斯顿大学的[GitHub 仓库](https://github.com/PrincetonLIPS/numpy-hilbert-curve)，该仓库实现了约翰·斯金林在 2004 年发表的一篇名为[编程希尔伯特曲线](https://pubs.aip.org/aip/acp/article-abstract/707/1/381/719611/Programming-the-Hilbert-curve?redirectedFrom=PDF)的论文中的算法。不幸的是，这篇论文对公众并不免费，所以我决定分析普林斯顿仓库中的代码。
...
![](img/9153416c17b0e247b11651d8fefb9862.png)
...
Skilling 算法的下一步是从二进制到格雷码的转换。关键区别在于[格雷码](https://www.geeksforgeeks.org/dsa/gray-to-binary-and-binary-to-gray-conversion/)中，连续的数字只在一个位上不同。这一属性至关重要：它确保曲线可以平滑地从一个个象限移动到下一个（即使曲线在每个象限中的取向仍然不正确）。

通过观察二进制数和曲线不同部分的取向，我们可以看到曲线仍然不正确，但现在每个象限的末端现在连接到下一个象限的起始处。

![](img/921db479d340d831771124ee54e5397c.png)
...
Skilling 方法的真正“魔法”始于对格雷编码位的重新排序——这一步被称为*解缠*。在我们的 4×4 示例中，我们最初将四个位解释为*(bitx[1], bity[1], bitx[2], bity[2]*)，其中第一对编码主象限，第二对编码子象限。然而，对于坐标计算，我们需要一种形式为(bitx[1], bitx[2], bity[1], bity[2]*)的结构，以便所有 x 位和 y 位都可以稍后组合成相应的十进制坐标(x, y)。这一步被称为位的解缠。

![](img/7aaf11f5f3262f31400baa59d9259601.png)
...
图 6 说明了 4×4 情况下的这个过程。左侧的表格显示了如何通过应用简单的变换（**交换**和**位翻转**）将格雷编码坐标转换为标准二进制数。
...
![](img/742e08ab9da99545a38d298cfa899c2a.png)
...
1.  从最高维度（2D 中的 y）到最低维度的内循环

1.  : 查看当前位。如果是 1：翻转维度 0（通常是 x）中所有较低的位。如果是 0：交换当前维度和维度 0（如果它们不同）之间的值。

    在当前维度和维度 0（如果它们不同）的较低位之间交换值。

第 3 步可以通过使用 numpy 的 XOR 操作轻松计算。每次迭代中翻转和交换位的整个过程在以下动画中进行了可视化。

![图片](img/cc2a0a29f6f63bbbb035862f5a0b5dfe.png)

图 7：使用约翰·斯金林算法创建 2D 希尔伯特曲线的过程（图片由作者提供）

![图片](img/28a5049bd6eccfd50548154335867850.png)

图 8：使用约翰·斯金林算法创建 3D 希尔伯特曲线的过程（图片由作者提供）
...
![图片](img/f2329bc386b8682dc218328718306556.png)

图 9：数据映射到希尔伯特曲线点的过程。红色虚线箭头指示一些映射
...
![图片](img/119ab17adb21e8e0adf546c5e1108150.png)

图 10：使用希尔伯特曲线和 Z 曲线的 2D 设置的 100 个随机查询。如图所示，您什么也看不到！😉
...
![图片](img/837613c0c1c3ba7b5d3b9253069f6243.png)

图 11：基于簇数和簇内扩散的希尔伯特曲线与 Z 曲线的比较（2，3 和 4 维）。结果清楚地表明，希尔伯特曲线比 Z 曲线更好地保留了局部性（图片由作者提供）
...
[3] X. CHEN, “使用学习自适应曲线优化高维数据的块跳过,” SIGMOD, 第 3 卷, 2025\. [在线]. 可用:[`zheng-kai.com/paper/2025_sigmod_chen.pdf`](https://zheng-kai.com/paper/2025_sigmod_chen.pdf)
