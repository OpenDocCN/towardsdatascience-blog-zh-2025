...
### 使用来自 Postgres 数据库的源数据

作为多年的 Python 数据工程师，我一直没有参与数据仪表板的制作。这一切都改变了，当基于 Python 的库如 Streamlit、Gradio 和 Taipy 出现时。

有了它们的引入，Python 程序员没有借口***不***使用它们来制作美观的前端和仪表板。

在那时之前，唯一的其他选择是使用像 Tableau 或 AWS 的 Quicksight 这样的专用工具，或者——更糟糕的是——用 CSS、HTML 和 JavaScript 手动操作。

因此，如果你之前从未使用过这些新的基于 Python 的图形前端库之一，这篇文章就是为你准备的，因为我将带你通过使用最流行的库之一**`Streamlit.`**来编写数据仪表板的代码。

> 我的意图是这将是一系列关于使用三个最受欢迎的基于 Python 的 GUI 库开发数据仪表板的系列文章的第一部分。除了这篇文章，我还计划发布关于 Gradio 和 Taipy 的文章，所以请留意。我会尽可能在每一个仪表板中复制相同的布局和功能。我也会使用完全相同的数据，尽管格式不同，例如 CSV、数据库等 …
> 
> 请注意，我与 Streamlit/Snowflake、Postgres 或本文中提到的任何其他公司或工具都没有任何联系或隶属关系。

### 什么是 Streamlit？

Streamlit 由 Adrien Treuille、Amanda Kelly 和 Thiago Teixeira 于 2018 年创立，当它推出开源 Python 框架以简化交互式数据应用的创建时，迅速在数据科学家和机器学习工程师中获得了人气。

2022 年 3 月，数据云公司 Snowflake 收购了 Streamlit 及其功能被整合到 Snowflake 生态系统中，以增强数据应用开发。

Streamlit 的开源框架已被广泛采用，下载量超过 800 万次，使用该平台构建的应用程序超过 150 万个。一个活跃的开发者和贡献者社区继续在它的持续发展和成功中扮演着重要角色。

### 我们将要开发的内容

我们将要开发一个数据仪表板。我们的仪表板源数据将存储在一个单一的 Postgres 数据库表中，包含 10 万个合成销售记录。

实际上，数据的来源并不***那么***重要。它可以是文本或 CSV 文件，SQLite，或者你可以连接到的任何数据库。我选择 Postgres 是因为我在本地 PC 上有一份副本，使用起来很方便。
...
+   顶行允许用户通过日期选择器和下拉列表分别选择特定的起始和结束日期以及/或产品类别。

+   第二行 – 关键指标 – 显示了所选数据的顶层摘要。

+   可视化部分允许用户从三个图表中选择一个来显示输入数据集。

+   原始数据部分正是其名。这是所选数据的表格表示，实际上是在查看底层 Postgres 数据库表数据。

使用仪表板很简单。最初，显示整个数据集的统计数据。然后，用户可以使用显示顶部的 3 个选择字段来缩小数据焦点。图表、关键指标和原始数据部分会动态更改，以反映用户的选择。

### 基础数据

如前所述，仪表板的源数据包含在一个单一的 Postgres 数据库表中。数据是一组 100,000 条合成销售相关数据记录。以下是参考的 Postgres 表创建脚本。
...
这里有一些你可以用来为自己生成数据集的 Python 代码。首先确保已经安装了 numpy 和 polars 库。
...
我使用 Miniconda 来做这件事，但你可以使用最适合你的任何方法。

如果你打算走 Miniconda 的路线并且还没有安装，你必须首先安装 Miniconda。你可以通过这个链接获取它，

> [**Miniconda – Anaconda 文档**](https://docs.anaconda.com/miniconda)

环境创建完成后，使用 **`activate`** 命令切换到该环境，然后使用 **`pip install`** 安装我们所需的 Python 库。
...
我们首先导入所有我们将需要的外部库。接下来，我们设置一个 **ThreadedConnectionPool**，它允许多个线程共享数据库连接池。随后是两个辅助函数，一个用于获取数据库连接，另一个用于释放它。对于简单的单用户应用程序来说，这是多余的，但在 Web 应用程序环境中处理多个同时访问数据库的用户或线程时是必需的。
...
**get_date_range** 函数执行 SQL 查询以找到 `order_date` 列中的日期范围（`MIN` 和 `MAX`），并将两个日期作为元组返回：`(start_date, end_date)`。

**get_unique_categories** 函数运行一个 SQL 查询以从 `categories` 列中获取唯一值。在返回列表之前，它将类别名称（首字母大写）。

**get_dashboard_stats** 函数执行一个包含以下部分的 SQL 查询：

+   **`category_totals`**：计算给定日期范围内的每个类别的总收入。

+   **`top_category`**：找到营收最高的类别。

+   **`overall_stats`**：计算总体统计数据：

+   总营收（`SUM(price * quantity)`）。
...
+   `total_revenue`：指定期间的总营收。

+   `total_orders`：不同订单的数量。

+   `avg_order_value`：每订单平均营收。

+   `top_category`：营收最高的类别。
...
**get_plot_data** 函数在给定的日期范围和类别内获取每日营收。它按天（`DATE(order_date)`）分组数据，并计算每日营收（`SUM(price * quantity)`），然后返回一个包含列：`date`（日期）和 `revenue`（该日总营收）的 Pandas DataFrame。

**get_revenue_by_category** 函数在指定的日期范围内按类别获取营收总额。它按 `categories` 分组数据，并计算每个类别的营收（`SUM(price * quantity)`），按营收降序排列结果，并返回一个包含列：`categories`（类别名称）和 `revenue`（类别总营收）的 Pandas DataFrame。

**get_top_products** 函数在给定的日期范围和类别内检索按营收排名前 10 的产品。它按 `product_names` 分组数据，并计算每个产品的营收（`SUM(price * quantity)`），按营收降序排列产品，并在返回前将结果限制为前 10 个，返回一个包含列：`product_names`（产品名称）和 `revenue`（产品总营收）的 Pandas DataFrame。

**get_raw_data** 函数在指定的日期范围和类别内获取原始交易数据。

**plot_data** 函数接收一些数据（在一个 pandas DataFrame 中）以及你想要在 x 轴和 y 轴上绘制的列名。然后它创建一个条形图——垂直或水平，取决于选择的朝向——标注坐标轴，添加标题，并返回完成的图表（一个 Matplotlib 图形）。如果数据为空，它将显示“没有可用数据”的消息，而不是尝试绘制任何内容。
...
此代码部分创建了在 Streamlit 仪表板中显示关键指标的主要结构。它：

1.  **设置页面标题**：“销售绩效仪表板。”

1.  **提供**开始/结束日期和类别选择的过滤器。

1.  **从数据库中检索**所选过滤器的指标（如总营收、总订单等）。

1.  **应用自定义 CSS** 以样式化这些指标，使其在一个带有标签和值的框行中。

1.  **在 HTML 块中显示**指标，确保每个指标都得到自己的样式容器。
...
本节向仪表板这部分添加了一个标题为“可视化”的标题。它创建了三个标签页，每个标签页都显示数据的不同图形表示：

标签页 1：营收随时间变化

+   使用 `get_plot_data()` 获取按给定过滤器分组的**按日期分组的收入数据**。
...
+   使用 `get_revenue_by_category()` 获取按类别分组的**收入**。

+   调用 `plot_data()` 来创建按类别划分的**收入条形图**。

+   在第二个选项卡中显示图表。

选项卡 3：顶级产品

+   使用 `get_top_products()` 获取给定过滤器的**按收入排名前 10 的产品**。
...
最后一个部分以数据框的形式显示原始数据。用户可以上下滚动以查看所有可用的记录。

在末尾添加了一个空的 `st.write("")` 以提供更好的视觉对齐空间。

### 运行应用

假设您将代码保存到一个名为 app.py 的文件中。您可以通过以下命令行运行它，

```py
(streamlit_test) C:Usersthoma> python -m streamlit run app.py
```

如果一切按预期工作，您在运行上述命令后应该会看到这个结果。
...
点击显示的本地 URL，应该会弹出一个浏览器窗口，其中运行着 Streamlit 应用。

### 摘要

在这篇文章中，我试图提供一个全面的指南，介绍如何使用 Streamlit 和 Postgres 数据库表作为数据源来构建交互式销售绩效仪表板。

Streamlit 是一个基于 Python 的现代开源框架，它简化了数据驱动仪表板和应用程序的创建。我开发的仪表板允许用户通过日期范围和产品类别筛选数据，查看总收入和表现最佳类别等关键指标，探索收入趋势和顶级产品等可视化，并通过分页导航原始数据。

本指南包括一个完整的实现，从设置带有样本数据的 Postgres 数据库到创建用于查询数据、生成图表和处理用户输入的 Python 函数。这种逐步方法展示了如何利用 Streamlit 的功能来创建用户友好的动态仪表板，非常适合想要构建交互式数据应用的数据工程师和科学家。

虽然我使用了 Postgres 作为我的数据，但应该很容易修改代码以使用 CSV 文件或任何其他关系型数据库管理系统（RDBMS），如 SQLite，作为您的数据源。

* * *

> _ 到此为止。我希望您觉得这篇文章有用。如果您觉得有用，请访问我的个人资料页面[这个链接](https://medium.com/@thomas_reid)。从那里，您可以查看我的其他已发布的故事并订阅以获取新内容的通知 _。

如果您喜欢这个内容，Medium 认为您也会对以下文章感兴趣。
