...
![Muhammad Firdaus Abdullah 在 Unsplash 上的图片](img/cbb51a7bc2ad7564a3fb4a47eb4e4c60.png)

图片由[Muhammad Firdaus Abdullah](https://unsplash.com/@firi?utm_source=medium&utm_medium=referral)在[Unsplash](https://unsplash.com?utm_source=medium&utm_medium=referral)提供

由于 LLMs 的非确定性，很容易得到不符合我们应用意图的输出。一个著名的例子是 Tay，微软的聊天机器人，它因发布攻击性推文而闻名。

每当我正在开发一个 LLM 应用并想要决定是否需要实施额外的安全策略时，我喜欢关注以下几点：

+   **内容安全**：降低生成有害、偏见或不适当内容的几率。
...
在这个背景下，为代理实施安全措施意味着确保代理的第一个输出不是最终答案。

从技术角度讲，你希望根据特定的约束来评估代理的输出，并在必要时强制代理重新生成答案，直到它满足你的要求。

例如，想象一个应用，它会总结你过去一个月收到的所有电子邮件。你指定了个人信息，如发件人姓名，必须匿名化。然而，由于 LLMs 的不确定性，它们有时会“忘记”这个条件。在这种情况下，你可以依赖额外的步骤来验证你是否满足了严格条件，因为这是一个关键要求。

## CrewAI 简介

[CrewAI](https://docs.crewai.com/introduction)是我与代理合作时的首选框架。它简单、社区强大、开源，并且完全专注于代理。它还附带了许多额外功能，使其成为一个非常有吸引力的选择。

在深入探讨如何使用 CrewAI 实施安全措施之前，让我简要介绍一下我们将要使用的主要组件。

### 代理、任务和团队

在 CrewAI 中，**[任务](https://docs.crewai.com/concepts/tasks)**和**[代理](https://docs.crewai.com/concepts/agents)**本身之间有一个清晰的分离。这种分离允许你将原本可能包含在单个大型提示中的内容解耦。通过保持这两个概念的不同，你可以清楚地定义代理的角色以及代理应该做什么。
...
最后一步是执行我们的 Crew，使用传递给任务的输入参数——在这种情况下，是健身目标和历史体重数据。

### 流

我们刚刚介绍的内容足以构建一个简单的 Crew，但不足以创建一个具有护栏的完整功能代理应用。我们需要掌握的下一个概念是**[CrewAI 流](https://docs.crewai.com/concepts/flows)**。

流背后的想法是轻松创建具有链式任务、无缝状态管理、事件驱动响应和灵活控制流（条件、循环和分支）的动态 AI 工作流。

以下装饰器有助于操作执行流程：

+   **`@start()`**：标记流的入口点，并在流开始时启动任务。

+   **`@listen()`**：在特定任务或事件完成时执行一个方法。

+   **`@router()`**：根据条件或结果将流导向不同的路径。
...
此外，流允许访问一个共享对象，该对象存储和管理数据（`ExampleState`），使得任务之间能够无缝通信，并在整个工作流中保持上下文。

## CrewAI 流中的护栏

通过我们刚刚覆盖的两个简单概念，我们准备好增强我们的 AI 代理。在这个例子中，我将演示如何创建一个多代理 AI 应用，该应用能够生成文本，并在提供输出之前验证文本是否包含暴力内容。

文本中暴力内容的检查发生在应用程序内部。多亏了流的状态管理，我们可以控制文本重新生成的频率，防止无限循环。这种方法为本质上非确定性的东西引入了确定性。

### 导入

主要使用的库是**CrewAI**，但我们还将导入**[Pydantic](https://docs.pydantic.dev/latest/)**来创建一个`BaseModel`类，因为 CrewAI 的文档要求这样做。

> 对于这个例子，我们将假设已经根据前面的描述创建了两个代理及其相应的任务：一个用于生成文本，另一个用于检查文本中的暴力内容。
...
**[状态](https://docs.crewai.com/concepts/flows#flow-state-management)**是我们用来在整个流程执行过程中持久化数据的地方。我们需要持久化的第一条信息是生成的文本。这是我们存储文本跨所有迭代的地方，并在必要时更新它。
...
为了控制流程，我们还将使用一个标志来指示文本是否包含暴力，以及一个计数器来跟踪剩余的生成尝试次数。这些属性将在流程执行期间被访问和更新。

### 流程类

我们正在实现的一个类包含用于控制应用程序流程的方法。让我们一步步构建它，从`__init__`函数开始。
...
到目前为止，一切照旧。我们只是初始化了我们类的唯一属性。这里的新颖之处在于我们传递给超类`Flow`的`ViolenceCheckState` Pydantic 模型。此模型将代表我们的状态。
...
第一个方法必须使用`@start`装饰器来定义我们流程的起点。它后面跟着第二个方法，该方法在第一个方法之后执行。
...
这里开始变得有趣。现在我们使用`@router`装饰器根据应用程序是否需要再生文本来指导流程。这个决定基于`self.state.contains_violence`的值。
...
根据装饰器的指定，`route_text_validation`方法在`validate_text_for_violence`之后立即执行。它检查状态中的`contains_violence`属性：

+   如果没有检测到暴力，则返回`"safe"`。

+   如果检测到暴力和剩余尝试次数，则触发文本的再生。

### 处理信号

我们类的最后部分包括三个方法，它们监听来自`route_text_validation`的特定信号：`"safe"`、`"regenerate"`和`"not_feasible"`。
...
+   **`"safe"`**：如果没有检测到暴力，生成的文本将被保存到文件中。

+   **`"regenerate"`**：如果触发文本再生，`generation_attempts_left`计数器将递减，并且将再次调用`generate_text`方法，重新启动流程。

+   **`"not_feasible"`**：当达到最大再生尝试次数（在本例中为 2）时发生，并且应用程序无法生成无暴力的文本。

## 结论

这种方法对我来说是救命稻草。实现重点在于组织循环和处理迭代，而不是重新发明应用程序流程背后的逻辑。在这个例子中，我专注于检查文本是否包含暴力内容，并在发现暴力内容时确定适当的行动。多亏了框架提供的这个干净、内置的功能，我无需浪费时间管理流程逻辑本身。

### 更多用例

在众多使用这种方法的用例中，我想强调查询注入。这比你想象的要常见，尤其是在“提示工程”等角色日益凸显的情况下。使用 CrewAI，你可以创建一个流程，评估初始查询以确定攻击者是否试图利用聊天机器人，例如。

### 挑战

这是框架的方式：它给你很多，但当你出错时，它可能会加倍地拿走一切。我曾开始简单的应用，结果发现自己在工作数小时后，在 GitHub 上提交了修复错误的 pull 请求。

话虽如此，它并没有听起来那么糟糕。CrewAI 仍然是我首选的多代理应用。然而，就像任何新的库一样，依赖它意味着接受风险，并准备好应对它们。
