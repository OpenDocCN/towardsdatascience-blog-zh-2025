...
+   您可以从简单的**提示工程技巧**开始——添加更详细的指令，使用少量样本提示，或要求模型逐步思考。

+   如果准确性仍然不足，您可以加入一个**自我反思步骤**，例如，从 API 调用中返回错误并要求 LLM 纠正错误。

+   下一个选项是使用**RAG（检索增强生成）**为 LLM 提供最相关的上下文，以进一步提高精确度。

在我之前的 TDS 文章中，我们探讨了这种方法，*["从原型到生产：提高 LLM 的准确性"](https://medium.com/towards-data-science/from-prototype-to-production-enhancing-llm-accuracy-791d79b0af9b)。在那个项目中，我们构建了一个 SQL 代理，从 0%有效的 SQL 查询到 70%的准确性。然而，我们使用提示所能达到的极限是有限的。为了突破这一障碍，达到准确性的下一个前沿，我们需要采用更先进的技术。

最有前途的选项是微调。通过微调，我们可以从仅依赖提示中的信息转变为直接将附加信息嵌入到模型的权重中。

## 微调

让我们先了解什么是微调。微调是通过在较小的、特定任务的数据集上训练预训练模型来精炼它们的过程，以增强它们在特定应用中的性能。基本模型最初是在大量数据上训练的，这使得它们能够发展对语言的广泛理解。然而，微调将这些模型定制为特定任务，将它们从通用系统转变为高度针对性的工具。例如，指令微调教会了 GPT-2 进行聊天和遵循指令，这就是 ChatGPT 出现的原因。

基本 LLM 最初是训练用来根据大量文本语料库预测下一个标记的。微调通常采用监督方法，模型被展示特定的疑问和相应的答案，允许它调整其权重以提高准确性。

从历史上看，微调需要更新所有模型权重，这种方法被称为全微调。这个过程计算成本高昂，因为它需要在内存中存储所有模型权重、状态、梯度和前向激活。为了解决这些挑战，引入了参数高效的微调技术。PEFT 方法只更新模型参数的一小部分，而将其他部分冻结。在这些方法中，最广泛采用的一种是[LoRA](https://github.com/microsoft/LoRA)（低秩适应），它在不牺牲性能的情况下显著降低了计算成本。
...
+   微调使模型能够学习并保留比仅通过提示提供的信息多得多的信息。

+   它通常给出更高的准确性，经常超过 90%。

+   在推理过程中，它可以通过使用较小的、特定任务的模型而不是较大的通用模型来降低成本。

+   微调的小型模型通常可以部署在本地，从而消除了对 OpenAI 或 Anthropic 等云提供商的依赖。这种方法降低了成本，增强了隐私，并最小化了对外部基础设施的依赖。
...
+   结果的质量很大程度上取决于高质量训练数据的可用性。

由于这个项目专注于获取知识，我们将继续进行微调。然而，在现实世界的场景中，评估微调的好处是否足以证明所有相关成本和努力是重要的。

### 执行

下一步是规划我们如何进行微调。在听了["提高 LLM 应用准确性的方法](https://www.deeplearning.ai/short-courses/improving-accuracy-of-llm-applications/)课程后，我决定尝试[Lamini 平台](https://www.lamini.ai/)，以下是我的原因：

+   它提供了一个简单的单行 API 调用，用于微调模型。由于我们刚开始学习一项新技术，所以这特别方便。

+   虽然它不是免费的，对于玩具项目来说可能相当昂贵（每微调步骤 1 美元），但他们在注册时提供免费额度，这对于初始测试是足够的。

+   Lamini 实施了一种新的方法，即 Lamini Memory Tuning，它承诺在保留一般能力的同时，不会丢失事实准确性。这是一个重大的声明，值得尝试。我们将在稍后更详细地讨论这种方法。

当然，还有许多其他的微调选项可以考虑：

+   [Llama 文档](https://www.llama.com/docs/how-to-guides/fine-tuning/)提供了许多微调的食谱，这些食谱可以在云服务器上执行，甚至对于较小的模型可以在本地执行。

+   在线有许多逐步指南，包括如何在 Kaggle 上微调 Llama 的教程，来自[DataCamp](https://www.datacamp.com/tutorial/fine-tuning-llama-3-1)。

+   您不仅可以微调开源模型。OpenAI 还[提供](https://platform.openai.com/docs/guides/fine-tuning#which-models-can-be-fine-tuned)了微调其模型的能力。

### Lamini 记忆微调

如我之前提到的，Lamini 发布了一种新的微调方法，我相信值得更详细地讨论。

Lamini 引入了混合记忆专家（MoME）方法，该方法使 LLM 能够在几乎不损失的情况下学习大量事实信息，同时保持泛化能力，并需要可行的计算资源量。

为了实现这一点，Lamini 通过添加大量（约 100 万）LoRA 适配器以及一个交叉注意力层来扩展预训练的 LLM。每个 LoRA 适配器都是一个记忆专家，作为模型的一种记忆形式。这些记忆专家在各个方面具有专长，确保模型能够从其微调的数据中保留忠实和准确的信息。受信息检索的启发，这 100 万个记忆专家相当于模型智能检索和路由的索引。

在推理时间，该模型在每个层检索最相关的专家子集，并将其合并回基础模型以生成对用户查询的响应。

![来自 Li 等人 2024 年论文的图 | 来源](img/8ca6a10ad49b82e53cb8061bb13802ab.png)

来自 Li 等人 2024 年论文的图 | [来源](https://arxiv.org/abs/2406.17642)

Lamini 记忆微调据说可以达到 95%的准确率。与传统指令微调的关键区别在于，这种方法不是优化所有任务的平均误差，而是专注于实现模型特别训练来记住的事实零误差。

![来自 Li 等人 2024 年论文的图 | 来源](img/360a6e825c3f3ac9e89043185972c964.png)

来自 Li 等人 2024 年论文的图 | [来源](https://arxiv.org/abs/2406.17642)

因此，这种方法允许 LLM 在保持平均误差的同时保留其泛化能力，并几乎完美地回忆起重要的事实。

> 对于更多细节，您可以参考 Li 等人（2024）的研究论文["Banishing LLM Hallucinations Requires Rethinking Generalization"](https://arxiv.org/abs/2406.17642)。

Lamini 记忆微调具有巨大的潜力——让我们看看它是否能在实践中实现其潜力。

## 设置

像往常一样，让我们从设置一切开始。正如我们讨论的，我们将使用[Lamini](https://docs.lamini.ai/)来微调 Llama，因此第一步是安装 Lamini 包。
...
要微调一个 LLM，我们首先需要一个数据集——在我们的案例中，是一组问题和答案（SQL 查询）的配对。汇编数据集的任务可能看起来令人畏惧，但幸运的是，我们可以利用 LLM 来完成这项工作。

准备数据集时需要考虑的关键因素：

+   **数据质量**至关重要，因为我们要求模型记住这些事实。

+   **示例的多样性**很重要，这样模型才能学会如何处理不同的情况。

+   使用**真实数据**而不是合成的数据更可取，因为它更好地代表了现实生活中的问题。

+   微调数据集的通常最小大小约为**1,000 个示例**，但高质量的数据越多越好。

### 生成示例

创建问答对所需的所有信息都存在于数据库模式中，因此对于 LLM 生成示例来说这将是一项可行的任务。此外，我有一个[代表性集](https://github.com/miptgirl/miptgirl_medium/blob/main/sql_agent_accuracy/rag_set.json)的问答对，我使用了 RAG 方法，我们可以将其作为有效查询的示例呈现给 LLM（使用少量提示技术）。让我们加载 RAG 数据集。
...
想法是迭代地向 LLM 提供模式信息和一组随机示例（以确保问题的多样性），并要求它生成一个新的、相似的但不同的问答对。
...
由于我们需要高质量的数据集，我更倾向于使用更先进的模型——`GPT-4o`——而不是 Llama。像往常一样，我将初始化模型并创建一个用于结构化输出的虚拟工具。
...
现在，让我们将所有内容组合成一个函数，该函数将生成一个问答对并创建一组示例。
...
我生成了 2,000 个示例，但实际上，在这个玩具项目中我使用了一个更小的数据集。因此，我建议将示例数量限制在 200-300 个。

### 清洗数据集

如我们所知，“垃圾输入，垃圾输出”，因此在微调之前清理 LLM 生成的数据是一个重要的步骤。

首先也是最重要的一步是确保每个 SQL 查询都是有效的。
...
没有无效的 SQL 查询，但有些问题返回超过 1,000 行。

![](img/b92fd819c5cd69c91d4c61a0ba5b7e7a.png)

尽管这些案例是有效的，但我们专注于一个具有聚合统计的[OLAP](https://en.wikipedia.org/wiki/Online_analytical_processing)场景，因此我只保留了返回 100 行或更少的查询。
...
为了解决这些重复项，我为每个问题只保留了一个答案。

```py
filt_new_qa_df = filt_new_qa_df.drop_duplicates('question') 
```

尽管我生成了大约 2,000 个示例，但我决定使用一个更小的 200 个问题-答案对的集合。使用更大的数据集进行微调将需要更多的调整步骤，并且成本更高。
...
让我们创建适合 Lamini 微调格式的响应。我们需要准备一个包含`input`和`output`键的字典列表。
...
现在，我们已经完全准备好进行微调。我们只需要选择一个模型并启动这个过程。我们将微调 Llama 3.1 8B 模型。
...
我们可以指定多个超参数，您可以在[ Lamini 文档](https://docs.lamini.ai/tuning/hyperparameters/)中找到所有详细信息。目前，我只将最重要的参数传递给函数：

+   `max_steps`：这决定了调整步骤的数量。文档建议使用 50 步进行实验，以获得初始结果而不花费太多钱。
...
现在，我们只需要等待 10-15 分钟，让模型训练，然后我们可以测试它。
...
值得注意的是，我们还在使用 Lamini 进行推理，并将为此付费。您可以在[这里](https://www.lamini.ai/pricing)找到有关费用的最新信息。

初看结果看起来很有希望，但我们需要更稳健的准确度评估来确认它。

此外，值得注意的是，由于我们已经针对我们的特定任务微调了模型，它现在始终返回 SQL 查询，这意味着我们可能不再需要使用工具调用来获取结构化输出。

### 评估质量

我们在[我之前的文章](https://medium.com/towards-data-science/from-prototype-to-production-enhancing-llm-accuracy-791d79b0af9b)中详细讨论了 LLM 准确度评估，所以在这里我将提供一个简要的回顾。

我们使用一组黄金问题-答案对来评估模型的质量。由于这是一个玩具示例，我已将集合限制为仅 10 对，您可以在[GitHub](https://github.com/miptgirl/miptgirl_medium/blob/main/sql_agent_accuracy/golden_set.json)上查看。

评估过程由两部分组成：

+   **SQL 查询有效性**：首先，我们检查 SQL 查询是否有效，这意味着 ClickHouse 在执行过程中不会返回错误。

+   **查询正确性**：接下来，我们确保生成的查询是正确的。我们使用 LLM 比较生成查询和真实查询的输出，以验证它们提供语义上相同的结果。

![](img/398cbafe1e17aa084a808d35892bec8c.png)
...
没有惊喜——没有银弹，这始终是一个迭代的过程。让我们调查出了什么问题。

### 深入错误

方法很简单。让我们逐个检查错误，了解我们为什么得到这些结果以及我们如何修复它们。我们将从第一个不成功的示例开始。

* * *

**问题**：哪个国家在 2024 年首次用户数量最多？
...
查询是有效的，但它返回了错误的结果。问题在于模型假设每个用户的第一个会话将始终有`session_id = 1`。由于 Lamini Memory Tuning 允许模型从训练数据中学习事实，让我们调查模型为什么做出这个假设。很可能，这是在我们的数据中。

让我们回顾所有提到`first`的示例。我将使用广泛的简单搜索标准来获得高级视图。

![](img/7f0a509a3af33e8f6ce4abf2a815537f.png)

如我们所见，没有提到`首次用户`的示例——只有关于第一季度的引用。模型无法捕捉这个概念并不令人惊讶。解决方案很简单：我们只需要添加一组关于首次用户的特定问题和答案的示例。

![](img/af751d2fc91cfb5a0273826dd90d228b.png)
...
我想讨论另一个错误案例，因为它将突出解决这些问题的过程中的一个重要方面。

**问题：**每个国家的购买收入的中间值和中位数四分位距（IQR）是多少？
...
在检查问题时，关注模型的误解或错误假设至关重要。例如，在这种情况下，可能会有添加与黄金数据集类似示例的诱惑，但这会过于具体。相反，我们应该解决模型误解的实际根本原因：

+   它很好地理解了中间值和中位数四分位距的概念。

+   按国家划分也是正确的。

+   然而，它误解了“购买收入”的概念，包括没有任何购买（`收入 = 0`）的会话。

因此，我们需要确保我们的数据集中包含足够与购买收入相关的信息。让我们看看我们现在有什么。只有一个示例，而且是不正确的。

![图片](img/3fb4315299029c51581aa2e8fa6b8189.png)

让我们修复这个示例并添加更多关于购买收入计算的案例。

![图片](img/1471fbd67e9a19f1a9c9223a9b59c5ed.png)

* * *

使用类似的方法，我为剩下的两个错误查询添加了更多示例，并编译了一个更新、清洗过的训练数据集版本。您可以在[GitHub](https://github.com/miptgirl/miptgirl_medium/blob/main/sql_agent_accuracy/small_sample_for_finetuning_cleaned.csv)上找到它。有了这个，我们的数据已经准备好进行下一轮迭代。
...
既然我们对数据有信心，我们可以进行微调。这次，我决定训练 150 步以达到更好的准确性。
...
在等待比上次更长一段时间后，我们现在有一个新的微调模型，在 150 次调整步骤后几乎损失为零。

![图片](img/8bc7e6143eb28997a8f38238fc7f432b.png)

我们可以再次运行评估，并看到更好的结果。因此，我们的方法是有效的。

![图片](img/c36ab4c4cbdfcac01261b569368c5569.png)

结果令人震惊，但仍然值得检查错误的示例，以了解出了什么问题。对于我们之前讨论的问题，我们得到了一个错误的结果："每个国家的购买收入的中位数和四分位距(IQR)是多少？"然而，这次，模型生成的查询与黄金集中的查询完全相同。
...
所以，问题实际上在于我们的评估。实际上，如果你尝试多次执行这个查询，你会注意到每次的结果都有细微的差异。根本原因在于[ClickHouse](https://clickhouse.com/docs/en/sql-reference/aggregate-functions/reference/quantile)中的`quantile`函数使用蓄水池抽样来计算近似值，这就是为什么我们会看到不同的结果。我们本可以使用`quantileExact`来获得更一致的数据。

也就是说，微调使我们实现了 100%的准确率。尽管我们的玩具黄金数据集只包含 10 个问题，但这是一项巨大的成就。我们已经从原始 Llama 的零有效查询进步到了 RAG 和自我反思的 70%准确率，而现在，得益于 Lamini 内存调优，我们达到了 100%的准确率。

> 您可以在[GitHub](https://github.com/miptgirl/miptgirl_medium/blob/main/sql_agent_accuracy/sql_agent_fine_tuning.ipynb)上找到完整的代码。

## 摘要

在本文中，我们继续探索提高 LLM 准确性的技术：

+   在[上一篇文章](https://medium.com/towards-data-science/from-prototype-to-production-enhancing-llm-accuracy-791d79b0af9b)中尝试了 RAG 和自我反思之后，我们转向了一种更高级的技术——微调。

+   我们尝试了 Lamini 开发的内存调优，它使模型能够以几乎零错误率记住大量事实。

+   在我们的例子中，内存调优表现异常出色，我们在 10 个问题的评估集中实现了 100%的准确率。
...
本文灵感来源于 DeepLearning.AI 的["提高 LLM 应用准确率"](https://www.deeplearning.ai/short-courses/improving-accuracy-of-llm-applications/)短期课程。

* * *

**免责声明：** *我以任何方式都与 Lamini 无关。本文中表达的观点完全是我个人的，基于对 Lamini 平台的独立测试和评估。本文旨在教育目的，并不构成对任何特定工具或服务的推荐。*
