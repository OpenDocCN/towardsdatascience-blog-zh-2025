...
> 原文：[`towardsdatascience.com/understanding-flash-attention-writing-the-algorithm-from-scratch-in-triton-5609f0b143ea/`](https://towardsdatascience.com/understanding-flash-attention-writing-the-algorithm-from-scratch-in-triton-5609f0b143ea/)
> 
> 免费阅读[alexdremov.me](https://alexdremov.me/understanding-flash-attention-writing-the-algorithm-from-scratch-in-triton?utm_source=medium)

Flash Attention 是一种革命性的技术，它极大地加速了基于 Transformer 模型的注意力机制，其处理速度比原始方法快得多倍。通过巧妙地划分数据和最小化内存传输，它解决了大型语言模型经常遇到的著名的 GPU 内存瓶颈问题。

在这篇文章中，我们将深入探讨 Flash Attention 如何利用高效的*I/O 感知*来减少开销，然后通过在 Triton 中构建一个**块稀疏注意力内核**来更进一步。
...
在原始实现中，注意力需要**O(n²**)额外的内存和**O(n²**)的计算时间复杂度，其中**n**是序列长度。**这太多了！**

## **Flash Attention**

### **核心思想**

Flash 注意力（Flash Attention）的主要思想可以用[原始论文](https://arxiv.org/pdf/2205.14135?ref=alexdremov.me)中的一句话来概括：
...
Flash Attention 算法提出了一种**在瓦片中计算注意力**的方法，而不需要显式地实现注意力分数张量：

![](img/9b4d492b14be6c8cef5db7fffcc8e2c9.png)

> 💥 **不实现矩阵**意味着在任何给定时间，矩阵都没有在内存中以完整形状存在。
...
![瓦片 QK^T | 图片由作者提供](img/7ba9ca6f82a416f46eeeee1bcf38178d.png)

瓦片 QK^T | 图片由作者提供

因此，我们在 SRAM 中计算了 **QK^T**。剩下的只是应用 softmax，乘以 **V**，就完成了！

![](img/f5359b6ef738dac38cd587e0f4ea2eaf.png)
...
从原始文章中提取算法，我们可以定义规则来计算数据连接的 softmax。有两个向量 **x1** 和 **x2**，我们需要计算这些向量连接 **[x1, x2]** 上的 softmax 分母 **l(x)**。如果向量的最大值是 **m(x)**，我们可以很容易地推导出连接的 softmax 分母：

![](img/253f0901a46f6e1a72edd795432385f1.png)![](img/e15b8a29e6bc73c128bf035ea3d92ab0.png)

最后的等价性可以很容易地验证为

![](img/ebc9456a6c92bc7855aefd200a49e1c4.png)
...
![内核作业数据管理 | 图片由作者提供](img/c65237ed01b4a8de4c3c3d288b10fee7.png)

内核作业数据管理 | 图片由作者提供

总结来说，每个作业将加载一个单独的 **Q** 块，遍历 **K** 和 **V** 中的所有块，并存储一个与 **Q** 块对应的输出块。
...
上面的代码[经过广泛测试](https://github.com/alexdremov/kernels/blob/main/tests/test_self_attention.py?ref=alexdremov.me)以匹配 PyTorch 的 `scaled_dot_product_attention`。您也可以查看测试以了解如何使用编写的内核。
...
> [**kernels/benchmark/benchmark_self_attention.py 在 main · alexdremov/kernels**](https://github.com/alexdremov/kernels/blob/main/benchmark/benchmark_self_attention.py)
> 
...
> [**FlashAttention：具有 I/O 感知的快速且内存高效的精确注意力**](https://arxiv.org/abs/2205.14135?ref=alexdremov.me)
> 
> [**教程 – Triton 文档**](https://triton-lang.org/main/getting-started/tutorials/index.html?ref=alexdremov.me)
> 
> [**GitHub – alexdremov/kernels：有用的内核集合**](https://github.com/alexdremov/kernels/tree/main?ref=alexdremov.me)
