...
![图片](img/8f824d796fe0c4fc7e444e6476c86b1a.png)

CUB 200 数据集中的两种海鸥物种展示了**细粒度对象分类**的难度[4]。
...
![图片](img/a92f8e67cacf8fc7a603e393aa956291.png)

基于深度学习的细粒度图像分析（FGIA）景观概述 [2]。

当我们更仔细地查看最近的[基准结果](https://web.archive.org/web/20250317111655/https://paperswithcode.com/sota/fine-grained-image-classification-on-stanford)（来自 Papers with Code 的存档），许多表现最好的解决方案都是基于 transformer 架构。这些模型通常达到了最先进的准确率，但很少讨论推理时间或部署约束。鉴于我们的要求，我们相当确信这些模型在边缘设备上实时运行时不会表现良好，因为该设备已经并行运行了多个模型。

在这项工作的时间，斯坦福汽车的最好结果是 97.1%的准确率，由**CMAL-Net**实现。
...
虽然数据集的原始托管网站已不再可用，但它仍然可以通过如 Kaggle 和 Huggingface 等精选仓库访问。该数据集在**BSD-3-Clause 许可**下分发，允许商业和非商业使用。在本工作中，它仅用于研究背景，以产生此处展示的结果。

![](img/b5ff3750939edc2f7e935d2a14946139.png)
...
我们使用**Stanford Cars**数据集在预训练的**ImageNet**上训练一个**ResNet-50**模型。每个模型在一个单独的**RTX 4090 GPU**上训练**600 个 epoch**，基于验证准确率使用**200 个 epoch 的耐心**进行**早期停止**。
...
我们在运行之间固定**随机种子**以确保可重复性和减少实验之间的方差。为了评估配方变化的真正影响，我们遵循最佳实践，并在**多次运行（通常是 3 到 5 次）**上平均结果。
...
![](img/fe9064ec45745ddca9ea5076baf951c7.png)
...
![](img/90fa67a075e1bda05dfcf261ed9d36e6.png)
...
接下来，我们探索了修改学习率计划。我们切换到**余弦退火**策略，该策略通过余弦函数从初始值降低到 0 来降低学习率。余弦的一个大优点是没有任何超参数需要优化，这又减少了我们的搜索空间。

![](img/f436fd7d9633df2bfacbd31770c21e71.png)
...
有趣的是，我们通过实验发现，添加标签平滑可以减少训练过程中的梯度方差。这使得我们能够安全地增加学习率，而不会破坏训练过程。因此，我们将**初始学习率从 0.1 增加到 0.4**。
...
![](img/9dbb01ce67d51118ed65d896a987c3a3.png)
...
由于我们的目标是开发一个**通用训练方案**而不是针对单个数据集过度定制，我们选择**保留 EMA 在最终设置中**。在其他条件下，其优势可能更为明显，并且其低开销使其成为一个安全的选择。
...
+   **AutoAugment**：取得了强大的结果和有竞争力的准确度，但我们发现**TrivialAugment**更好。更重要的是，TrivialAugment 完全无参数，这减少了我们的搜索空间并简化了调整。

+   **替代优化器和调度器**：我们尝试了广泛的优化器和学习率调度器。**Nesterov 加速梯度（NAG）**在优化器中始终给我们带来最佳性能，而**余弦退火**作为最佳调度器脱颖而出，它提供了强大的结果，无需调整额外的超参数。

## 4. 结论：

下面的图表总结了我们在逐步构建训练方案过程中的改进：

![图片](img/3e12ef5bbac0292e87c4acd523ed087d.png)
...
[3] [Reslan 和 Farou。使用深度学习自动进行鸟类物种的细粒度分类。](https://www.researchgate.net/publication/366192757_Automatic_Fine-grained_Classification_of_Bird_Species_Using_Deep_Learning)

[4] [Zhao 等人。基于深度学习的细粒度物体分类和语义分割综述。](https://www.zhaobo.me/papers/ijac_survey.pdf)

[5] [He 等人。卷积神经网络图像分类的技巧汇总。](https://arxiv.org/pdf/1812.01187)

[6] [Lee 等人。在卷积神经网络中组合组装技术的性能改进。](https://arxiv.org/pdf/2001.06268)

[7] [Wightman 等人。ResNet 反击：Timm 中的改进训练过程。](https://arxiv.org/abs/2110.00476)

[8] [Vryniotis。如何使用 TorchVision 的最新原语训练最先进的模型。](https://pytorch.org/blog/how-to-train-state-of-the-art-models-using-torchvision-latest-primitives/)

[9] [Krause 等人，用于细粒度分类的 3D 对象表示。](https://ieeexplore.ieee.org/document/6755945)

[10] [Müller 和 Hutter。TrivialAugment：无需调整且最先进的数据增强。](https://arxiv.org/abs/2103.10158)

[11] [Zhong 等人，随机擦除数据增强。](https://arxiv.org/pdf/1708.04896)
