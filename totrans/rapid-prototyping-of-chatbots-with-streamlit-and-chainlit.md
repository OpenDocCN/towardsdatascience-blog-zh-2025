...
> 原文：[`towardsdatascience.com/rapid-prototyping-of-chatbots-with-streamlit-and-chainlit/`](https://towardsdatascience.com/rapid-prototyping-of-chatbots-with-streamlit-and-chainlit/)

*<mdspan datatext="el1758113565427" class="mdspan-comment">快速原型设计</mdspan>* <mdspan datatext="el1758162360901" class="mdspan-comment">指的是一个迭代过程，通过构建产品的简单版本并收集常规用户反馈，以快速验证重要假设和假设，并评估关键风险。这种方法与敏捷软件开发实践以及精益创业方法中的“构建-度量-学习”过程紧密一致，可以显著降低开发成本并缩短上市时间。快速原型设计对于推出成功的 AI 产品特别有用，考虑到相关技术、用例和用户期望的早期性质。*
...
前往[这里](https://ollama.com/download)下载并安装 Ollama。通过在终端运行此命令来检查安装是否成功：
...
接下来，我们将使用`uv`，一个快速且用户友好的 Python 项目管理工具，来设置项目目录。按照[这里](https://docs.astral.sh/uv/getting-started/installation/)的说明安装`uv`，并使用以下命令验证安装：
...
如果没有指定`--bare`选项，`uv`在初始化过程中会创建一些标准工件，例如`main.py`、`README.md`和一个 Python 版本固定文件，但对我们这些演示来说这些不是必需的。最小化过程只创建一个`pyproject.toml`文件。
...
我们将实现一个名为`LLMClient`的类，用于后端功能，该功能可以与以 UI 为中心的功能解耦，这是 Streamlit 和 Chainlit 等框架的关键区别。例如，`LLMClient`可以处理诸如在 LLM 提供商之间选择、执行 LLM 调用、与外部数据库交互以进行检索增强生成（RAG）以及记录对话历史以便后续分析等任务。以下是一个`LLMClient`的示例实现，保存在一个名为`llm_client.py`的文件中：
...
如果应用程序没有在您的默认浏览器中自动打开，请手动导航到 URL（[`localhost:8501`](http://localhost:8501)）。您应该看到一个基本的聊天界面。在提示字段中输入以下问题并按 Enter 键：

> *摄氏度转换成华氏度的公式是什么？*

图 1 显示了结果：

![](img/9c996bc2642d6596f6bced066c84bece.png)
...
![](img/62533ab1c934b4e6fc118bdfb4004f40.png)
...
![图片](img/e4f1da0d3dd50a0be9dab126838d19d0.png)

图 3：初始 Chainlit 问答

![图片](img/f9a4532f45c7474fa836a98058967b02.png)
...
![图片](img/728c2f0227bfec39fd68edfd13254c30.png)
...
![图片](img/a90ca9f6c7c551b80e76852eaf1cd7ab.png)
