...
![由 Max Chen 在 Unsplash 提供的图片](img/430b4741261efdb0baa82dc97c710ecb.png)

图片由 [Max Chen](https://unsplash.com/@maxchen2k?utm_source=medium&utm_medium=referral) 在 [Unsplash](https://unsplash.com?utm_source=medium&utm_medium=referral) 提供
...
![由 David Pupăză 在 Unsplash 提供的图片](img/18418e194042911a2f38ebd5c7553272.png)

图片由 [David Pupăză](https://unsplash.com/@davfts?utm_source=medium&utm_medium=referral) 在 [Unsplash](https://unsplash.com?utm_source=medium&utm_medium=referral) 提供
...
![由 Laura Gariglio 在 Unsplash 提供的图片](img/ff5b2e3130a6e139b78215168209cc7a.png)

Laura Gariglio 在[Unsplash](https://unsplash.com?utm_source=medium&utm_medium=referral)上的照片
...
`scikit-learn`的估计器套件是封装的理想例子。

> [**KMeans**](https://scikit-learn.org/1.5/modules/generated/sklearn.cluster.KMeans.html#sklearn.cluster.KMeans)

我们都知道`KMeans`是如何工作的以及它应该做什么。我们也知道如何使用`scikit-learn`对`KMeans`的实现——所有这些都记录在 docstrings 中。
...
但我们是否需要确切地知道`fit`和`predict`方法是如何实现的？我是否需要阅读`fit`中的所有 126 行代码来了解如何使用它？

**不**。**我们只需要知道它为我们给出的数据输出一些聚类。**

即使是新手数据科学家也能使用这个界面并在一定程度上理解它。这就是封装的点。

![Dan Crile 在 Unsplash 上的照片](img/4f348ae5a3124fe23e1af629b8960098.png)

Dan Crile 在[Unsplash](https://unsplash.com?utm_source=medium&utm_medium=referral)上的照片

作为用户，我们不需要知道引擎盖下发生的事情的细节。所有这些细节都整齐地藏在名为`fit`的有意义的功能之下。

我们需要知道的就是`fit`的预期输入和输出是什么，并适当地使用它。
...
1.  如果读者需要深入了解，他们可以自由地查看每个函数及其相应的文档字符串，并立即理解每个函数如何在可管理的、有意义的块中工作。

1.  如果他们不需要细节，他们可以快速跳转到与他们相关的部分，至少在较高层次上了解上述代码的意图。

这种方法假设笔记本中还有其他更重要的部分，例如模型构建或特征探索。因此，我觉得数据清理部分不是这段代码的主要焦点。

因此，我将每个函数命名为`clean_xxx_column`格式，假设读者只需要知道*“这里进行了一些清理以创建可工作的数据集”，*并且如果他们的兴趣在于模型构建/特征探索，他们可以跳过细节。
...
然而，这次你发现需要额外的清理步骤；`claps`列与`earnings`列存在相同的问题，其中一些值非常大：`9999999999`
...
![Pandu Agus Wismoyo 在 Unsplash 上的图片](img/d0e3e2aa5fc8ffb15a8afc27a9475130.png)

图片由[Pandu Agus Wismoyo](https://unsplash.com/@kangterbang?utm_source=medium&utm_medium=referral)在[Unsplash](https://unsplash.com?utm_source=medium&utm_medium=referral)提供
