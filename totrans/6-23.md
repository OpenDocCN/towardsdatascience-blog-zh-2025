...
HuggingFace 的 transformer 库提供了许多基本构建块和多种功能，以启动您的 AI 代码。许多产品和库都是基于它构建的，在这篇简短的博客中，我将讨论一些人们扩展它以在 HuggingFace transformer 库之上添加自定义训练代码的方法：

1.  **重新实现训练代码**，通过遍历训练数据来重新创建微调循环，然后添加自定义代码，并且

1.  **创建自定义回调**附加到 _[Trainer](https://huggingface.co/docs/transformers/en/main_classes/trainer)_ 类，以便将自定义代码添加到回调中。

显然，可能还有其他方式来自定义微调循环，但本博客旨在关注这两种方法。

## 重新实现用于自定义微调循环的训练代码

通常在训练模型时，会创建一个 _[Trainer](https://huggingface.co/docs/transformers/en/main_classes/trainer)_ 对象，该对象允许您指定训练模型的参数。Trainer 对象提供了一个 *train()* 方法，您可以通过调用该方法来启动训练循环：
...
一些库不是通过调用 Trainer 对象，而是通过以下方式添加自定义代码：1) 在 *train()* 函数中重新实现传递数据以微调模型的代码，然后 2) 在重新实现的各个点添加自定义代码。一个很好的例子是 AllenAI 的开源库 [Data Cartography](https://github.com/allenai/cartography/tree/main)。

在库中，*训练动态* - 模型和训练数据点的特征 - 在微调循环的每个训练周期后都会被捕获。捕获训练动态需要在微调循环中编写自定义 [代码](https://github.com/allenai/cartography/blob/3df3438fcc7324e706dee2e787426389bbd8fb1a/cartography/classification/run_glue.py#L204-L264)。在代码中，创建了一个迭代器，遍历每个训练周期，并为每个训练周期将训练数据的批次传递给模型进行训练（以下是一个简化和带注释的 [实现](https://github.com/allenai/cartography/blob/3df3438fcc7324e706dee2e787426389bbd8fb1a/cartography/classification/run_glue.py#L204-L264) 版本）:
...
通过重新实现微调循环内部的操作，**Trainer 对象的基本[部分](https://github.com/huggingface/trl/blob/v0.13.0/trl/trainer/base.py)也被重新实现了**，包括在训练数据批次上执行训练步骤和计算模型在数据批次上的损失。虽然这种自定义微调循环的方法让开发者对实现有更精细的控制，但这种方法也需要大量工作来确保代码能够正常工作。第二种添加自定义代码的方法不需要重新实现 Trainer 对象的部分，因为它使用 *自定义回调*。

## **创建自定义回调**以自定义 Trainer 类

回调是一个作为参数传递给另一个函数的函数。第二个函数可以在稍后调用传递的函数。回调允许你在传递给第二个函数的函数中添加自定义代码。有一个名为 _[TrainerCallback](https://huggingface.co/docs/transformers/v4.47.1/en/main_classes/callback#transformers.TrainerCallback)_ 的类，它包含 [空的回调函数](https://github.com/huggingface/transformers/blob/241c04d36867259cdf11dbb4e9d9a60f9cb65ebc/src/transformers/trainer_callback.py#L260)，你可以用你的自定义代码覆盖它们。这些回调函数本质上是在 Trainer 类（或如果你使用的是 SFTTrainer 等继承版本，则在其内部）的训练循环的不同部分被调用的。
...
每个空函数都传递几个参数（1）*args* 类型为 TrainingArguments，(2) *state* 类型为 TrainerState，(3) *control* 类型为 TrainerControl，以及（4）组合到 *kwargs*** 参数中的额外任意参数。这些参数包含当前对 Trainer 类有效的对象，你可以访问它们并添加自定义代码。更多关于这些参数的详细信息可以在 [这里](https://huggingface.co/docs/transformers/v4.47.0/en/main_classes/callback#transformers.TrainerCallback) 找到。

例如，如果你想在每个 epoch 后访问模型的当前状态，你就可以

1.  覆盖 _on_epoch*end* 函数并

1.  在 _on_epoch*end* 函数中访问模型
...
在上面的代码中，我们使用 *kwargs["model"]* 行在每个 epoch 结束后访问模型的当前状态，然后添加一些自定义代码（在这种情况下，我们在 ExampleTrainerCallback 类的 init 函数中对一些数据集进行 tokenization 后进行推理）。使用 TrainerCallback 的好处是，我们不必深入到 Trainer 类中一些核心操作的重新实现，包括计算损失和将训练数据批次传递到模型中进行学习。我们通过使用现有的代码（其他人已经测试过的代码）来保留所有这些操作，并在现有代码的基础上添加我们的自定义代码。

自定义回调的一个主要问题是确保你也将它传递给将要使用的 Trainer 对象：
...
**旁注：**在使用 TrainingArgument 类（随后传递给 Trainer 类）进行微调时，权重和偏差有时默认启用。如果你使用专有数据（也取决于你所在组织的政策），则应将其禁用：
