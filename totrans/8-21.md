...
> 原文：[`towardsdatascience.com/speed-up-pandas-code-with-numpy-fdee76210794/`](https://towardsdatascience.com/speed-up-pandas-code-with-numpy-fdee76210794/)

![图像由 AI（Dalle-3）生成](img/0b58b0660e82625f817aeda184a5a99b.png)

图像由 AI（Dalle-3）生成

在我最初在 Medium 上写的一篇文章中，我谈到了在 Pandas 数据框上使用 apply()方法，并说如果可能的话，应该避免在较大的数据框上使用它。如果你想查看这篇文章，我会在文章末尾提供链接。

尽管我那时谈到了一些可能的替代方案，即使用向量化，但我没有给出很多使用向化的例子，所以我打算在这里纠正这一点。具体来说，我想谈谈如何使用 NumPy 及其一些不太为人所知的函数**（`where`**和**`select`**）来加速涉及复杂 if/then/else 条件的 Pandas 操作。

在 Pandas 的上下文中，向量化指的是一次对整个数据块应用操作，而不是逐行或逐元素迭代。这种方法之所以可行，是因为 Pandas 依赖于 NumPy，它支持高度优化的向量化操作，这些操作是用 C 编写的，从而实现了更快的处理。当你使用 Pandas 中的向量化操作时，例如对 DataFrame 或 Series 对象应用算术运算或函数，操作会同时发送到多个数据元素。

这不仅使代码更加简洁易读，而且可以通过减少 Python 循环的开销并利用现代 CPU 并行处理多个数据点的功能来显著提高性能。向量化是使 Pandas 在 Python 中进行数据处理和分析变得强大的关键特性。

问题在于，一些向量化操作是显而易见的，甚至在你没有意识到的情况下就已经在底层完成了。然而，很多时候，当你想要执行的操作类型变得更加复杂时，很难看到在这些情况下如何应用向量化。

在这篇文章中，我将讨论一些常见场景，在这些场景中这是适用的，并展示如何应用向量化。

### 获取一些数据来工作

在我许多关于 Python 库和数据库系统性能的文章中，我使用一组合成销售数据进行测试。这个数据集的模式看起来像这样，
...
这里有一个你可以使用的 Python 程序来生成这样的数据集。它生成一个 CSV 文件。要创建的记录数量和输出文件的位置是可以配置的。它使用 NumPy 和 polars 库，所以在运行之前你必须安装这些库。

对于这次测试，由于我有一台相当高性能的 PC，我将创建并使用一个包含 100 万条记录的 CSV 文件。
...
在我们进入示例代码之前，让我们设置一个独立的开发环境。这样，我们所做的工作就不会干扰我们可能正在为其他项目进行的其他版本库、编程等。

我使用 Miniconda，但您可以使用最适合您的方法。

如果您想走 Miniconda 路线并且还没有安装它，您必须首先安装 Miniconda。您可以通过此链接获取它，
...
现在在命令提示符中输入 `jupyter notebook`。您应该在浏览器中看到一个 jupyter notebook 打开。如果它没有自动发生，您可能会看到 `jupyter notebook` 命令之后的一屏信息。

在底部附近，将有一个您应该复制并粘贴到浏览器中以启动 Jupyter Notebook 的 URL。
...
首先，让我们将数据集读入 Pandas 数据框中。我们将计时以了解我们获得的任何速度提升。
...
因此，您在 Pandas 数据框上执行的大多数操作本质上都是向量化。例如，假设我们想要将数量字段乘以 5 并更新总价值列。
...
处理 100 万条记录不到百分之一秒。我们如何知道上述是向量化过程？除了运行所需的时间极短之外，我们还可以检查使用非向量化方法（特别是使用 apply() 方法）完成相同任务所需的时间。

如果您在 Pandas 编码方面不是很熟练，这可能是您最初解决此问题的方法。

**使用 Apply() 方法**

Apply() 允许您运行一个函数，该函数将应用于数据框中的每一条记录。这应该比使用 for 循环遍历数据框行要快，但它与原始向量化代码相比如何？
...
因此，正如您从上述操作的计时中可以看到的，我们已经确定在处理大型数据集时，Pandas 编码中的向量化是必不可少的，而且它通常只是为简单的代码问题在“幕后”实现。

但当我们的编码需求稍微复杂一些时会发生什么？

### 示例 2 - 向量化 If/then/else 条件

在这个例子中，假设我们想要执行之前的相同操作（数量 *5），但这次只为智能手机产品。对于任何其他产品，我们想要（数量* 2）。

这里是 naive apply() 实现的示例。
...
我们可以通过向量化来提高这一点。第一种方法是显而易见的，也可能是大多数经验丰富的 Pandas 程序员会转向的方法，因为它看起来很自然。这也很快。
...
我将要展示的第二种方法使用的是你可能之前没有遇到过的 NumPy 方法——NumPy 的**where()**函数，而且它的速度甚至比常规向量化还要快。Numpy.where()函数接受三个参数。第一个参数是你正在测试的条件。第二个参数是在测试条件返回 True 时返回的内容，第三个参数是在测试条件为 False 时返回的内容。
...
在这种情况下，使用 NumPy 比常规向量化快 5 倍以上，比使用 apply()快 3500 倍。

### 示例 3 – 向量化多个 if/then/else 条件。

可以使用嵌套的 Numpy.where 条件来处理嵌套的 if/then/else 条件。但是，当存在许多此类情况时，代码的可读性和可维护性会变得困难。如果你曾经在 Excel 中使用过嵌套的 IF 语句，你就会明白我的意思。

在这种情况下，我们可以使用另一个相当酷的 NumPy 方法，很多人可能都没有听说过，这个方法叫做**select()**。

就像 where()函数一样，select()函数也接受 3 个参数。第一个参数是一个 Python 列表，包含要测试的**`条件`**。第二个参数是一个 Python 列表，告诉 NumPy 在满足等价条件时应返回什么。第三个参数是**`默认`**语句，用于在没有任何条件满足时返回的内容。

这些顺序很重要。choices 列表中的第一个项目与 conditions 列表中的第一个项目相对应，依此类推。

在某种程度上，它就像在 Python、C、Java 等编程语言中使用的 case 语句一样。

将我们之前的例子推向极端。比如说，我们想要根据不同类型的产品乘以不同的数量。
...
### 示例 4 – 向量化嵌套的多个 if/then/else 条件

我的最后一个例子与之前的例子类似，但现在只有当智能手机的现有数量大于 5 时，智能手机的数量才会乘以 1.5。否则，数量会乘以 2。

其他条件保持不变。换句话说，我们有一个嵌套的 if/then/else 场景。如果这个逻辑在一个像 Python 这样的编程语言中的函数里，它看起来会像这样，
...
我们能否向量化这个？是的，我们可以通过修改之前的方法来实现。这次，我们在`choices`列表中添加了一些额外的布尔逻辑测试和额外的操作。
...
希望你能从这个文章中带走两点。第一点是，当处理中等或大型数据集时，向量化是高效 Pandas 代码的必要组成部分。第二点是，即使你需要应用到 dataframe 记录上的操作很复杂，它们仍然经常可以被向量化。

我解释了为什么在 Pandas 中进行向量化对于编写高效代码至关重要，并展示了我们如何在复杂的数据处理场景中使用向化的几个示例。

> _ 现在就到这里吧。希望你觉得这篇文章有用。如果你觉得有用，请访问我的个人资料页面[这里](https://medium.com/@thomas_reid)。从那里，你可以看到我其他发布的文章，并订阅以获取我发布新内容的通知。_
...
这里是关于在处理大型数据框时避免使用 apply()方法的故事链接。

> [**考虑在大型数据框上使用 Pandas apply()：停止！——请阅读这个替代方案**](https://towardsdev.com/avoid-using-the-pandas-apply-method-on-big-dataframes-51d8a4c072b7)
