...
> 原文：[`towardsdatascience.com/how-to-use-llms-for-powerful-automatic-evaluations/`](https://towardsdatascience.com/how-to-use-llms-for-powerful-automatic-evaluations/)

<mdspan datatext="el1755113810780" class="mdspan-comment">在这篇文章中，我</mdspan>讨论了如何使用 LLM 作为评判者进行自动评估。LLM 今天被广泛应用于各种应用。然而，LLM 的一个常被低估的用例是评估。使用 LLM 作为评判者，您利用 LLM 来评判输出的质量，无论是给它打 1 到 10 分，比较两个输出，还是提供通过/不通过反馈。本文的目标是提供如何利用 LLM 作为评判者来提高您自己应用程序开发效率的见解。

![图片](img/25b7d86ee565118c1329c67f6d59286a.png)

这张信息图表突出了我文章的内容。图片由 ChatGPT 提供。

您还可以阅读[我的关于基准测试 LLM 的文章](https://towardsdatascience.com/how-to-benchmark-llms-arc-agi-3/) [使用 ARC AGI 3](https://towardsdatascience.com/how-to-benchmark-llms-arc-agi-3/)，并查看[我的网站，其中包含所有我的信息和文章](https://eivindkjosbakken.com/)。
...
通过或失败是 LLM 作为裁判员常用的另一个评估指标。在这种情况下，你要求 LLM 裁判员根据构成通过和构成失败的标准来批准或拒绝输出。与评分评估类似，这个描述对于 LLM 裁判员的表现至关重要。再次，我建议使用示例，本质上利用少样本学习来提高 LLM 裁判员的准确性。你可以在[我的关于上下文工程的文章中了解更多关于少样本学习的信息。](https://towardsdatascience.com/how-to-significantly-enhance-llms-by-leveraging-context-engineering-2/#:~:text=There%20are%20a%20lot%20of,to%20perform%20a%20task%20effectively.)
