...
第一个原因是 LLM 没有足够的上下文来回答。这就是检索增强生成（RAG）出现的原因。RAG 为 LLM 提供它训练中未见过上下文。

一些模型在提供上下文以生成答案时表现良好，而另一些则不行。例如，[LLama 3.1 8B](https://ai.meta.com/blog/meta-llama-3-1/) 如果提供上下文生成答案，表现良好，而 [DistilBERT](https://huggingface.co/docs/transformers/en/model_doc/distilbert) 则不行。

> [**将 Tiny Llama 3.2 1B 微调以替代 GPT-4o**](https://towardsdatascience.com/i-fine-tuned-the-tiny-llama-3-2-1b-to-replace-gpt-4o-7ce1e5619f3d)

第二个原因是当答案需要一些推理时。并非每个 LLM 都擅长推理；每个 LLM 的做法都不同。为了说明这一点，[Llama 2 13B 在推理任务上的表现不如 GPT-4o](https://www.llm-reasoners.net/leaderboard)。

当然，这些模型来自不同的时代，将它们并排放在一起比较并不合适。但这也是我想说明的点——如果你没有明智地选择模型，你可能会期待它产生幻觉答案。

* * *

## 如何评估 RAG 中的幻觉

既然我们已经确信每个基于 LLM 的应用都可以产生幻觉，并且测量是唯一的方法来控制它，那么我们该如何做呢？
...
在这篇帖子中，我将使用 Deepeval。一个简短的说明：我与 Deepeval 没有关联；我只是喜欢它。

让我们从安装这个工具开始。您可以从 PyPI 仓库获取它。

```py
pip install -qU deepeval ragas
```

由于 LLM 评估是模糊的，我们不能像测试软件那样测试 LLM。LLM 生成的响应永远不可预测，因此需要另一个 LLM 来评估它们。

LLM 评估者需要是一个有能力的模型。我会选择 GPT-4o-mini 来做这个。它既经济又准确，但您可以尝试其他模型。

我们可以通过两种不同的方式来评估 RAGs。第一种是一种基于提示的技术，称为**[G-Eval](https://arxiv.org/abs/2303.16634)**。第二种是**RAGAS，它使我们能够系统地评估 RAGs**。

RAGAS 和 G-Eval 都是评估 RAGs 的极其有用的框架。当在这两个框架之间选择时，我会使用 RAGAS 作为默认方法。由于计算是特定的，结果很容易与不同的评估进行比较。然而，评估标准有时可能比 RAGAS 框架更复杂。在这种情况下，您可以使用 G-Eval 来指定评估步骤。

### G-Eval：更通用的评估框架

如前所述，G-Eval 是一个基于提示的评估框架，这意味着我们可以告诉 LLM 如何评估我们的 RAG。我们可以通过设置输入参数`criteria`或`evaluation_steps`来实现这一点。

这里是一个使用`criteria`参数的示例。
...
上述示例验证了实际输出与预期输出的一致性。在这种情况下，日期不同。但其他所有内容都是正确的。

G-Eval 的好处在于我们可以定义如何评估它。如果我们对日期差异小于 10 天的差异可以接受，我们可以在标准中指定。以下是新的评估方法。
...
正如您所注意到的，这使分数从.66 上升到.85，因为我们允许日期匹配的 10 天范围。

上述示例仅检查 LLM 对预期结果的响应是否正确。然而，我们还需要检查检索到的数据来评估 RAG。

这里有一个 RAG 评估示例。请注意，我们使用了`evaluation_steps`参数而不是`criteria`。
...
在上述示例中，我们使用了`evaluation_steps`而不是`criteria`。但这不是必需的。用单个语句`criteria`参数解释评估过程完全可以。但将它们分解成步骤总是更容易。

G-Eval 无论步骤如何，都提供了一个单一的评估分数。你可以给它一个单一的评估步骤，或者是一打步骤，G-Eval 将完成所有工作并产生一个单一的分数。

这很容易理解，通常也足够。但如果我们需要逐个测试 RAG 管道的组件呢？这正是 RAGAS 发挥作用的地方。

### RAGAS：标准和更细粒度的评估

RAGAS 是 RAG 管道中其他四个评估的组合。它们是答案相关性、忠实度、情境回忆和情境精确度。RAGAS 得分只是所有这些的平均值。

这里有一个例子：

在下面的例子中，我使用了`RagasMetric`和单个组件。在实际评估中，你可以使用单个组件或仅使用 RagasMetric。
...
为了理解这一点，让我们回顾一下 RAG 应用程序如何响应用户的输入。第一步是获取上下文信息。然后 LLM 使用检索到的上下文来回答用户的查询。

**情境预置**是一个度量标准，用于评估与输入相关的语句是否被排名更高。在我们的例子中，包含获取日期的那个语句在检索到的上下文中排名第三。因此，它在评估中的得分（0.33）较低。

**情境回忆**测试检索到的上下文是否足够接近预期的输出。在我们的例子中，获取日期在检索到的上下文中可用。因此，这个度量标准的得分是 1.0。

**忠实度**是衡量响应中幻觉的度量标准。它检查实际输出与检索到的上下文之间的正确性。在我们的例子中，检索到的上下文明确指出获取是在 2024 年完成的。但输出说是在 2025 年，这非常错误。因此，它在忠实度得分上得到了 0 分。

最后，**答案相关性**度量标准衡量生成的响应是否至少是尝试回答正确问题（输入）。尽管事实上不正确，但在我们的例子中，LLM 回答了正确的问题，因此它得到了 0.99 的分数。

**RAGA 得分**是这四个单个度量标准的加权平均值。

RAGAS 的缺点是，有时即使响应不正确，你也会得到通过。这正是我们例子中的情况。用相差一年的年份回答获取日期并不是微不足道的。但仍然，它只是四个考虑的度量标准中的一个。因此，0.66 的整体得分远高于阈值。

因此，我建议使用单个度量标准来理解组件，而不是使用单个度量标准（如 RAGAS）来理解整个系统。这有助于更好地调试应用程序。
