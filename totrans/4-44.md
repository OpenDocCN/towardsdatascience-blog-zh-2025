...
![由 Towfiqu barbhuiya 在 Unsplash 上的照片](img/ba7c0da785420e1056cd3f83d99f4f21.png)
...
欢迎来到关于营销组合建模（MMM）系列的第三部分，这是一本实用指南，帮助你掌握 MMM。在这个系列中，我们将涵盖关键主题，如模型训练、验证、校准和预算优化，所有这些都将使用强大的**pymc-marketing** Python 包来完成。无论你是营销组合建模的新手还是想提高你的技能，这个系列都将为你提供实用的工具和见解，以改善你的营销策略。
...
在本系列的第三部分中，我们将探讨如何通过以下领域开始从我们的营销组合模型中获得商业价值：

+   为什么组织希望优化他们的营销预算？

+   我们如何利用营销组合模型的结果来优化预算？

+   使用**pymc-marketing**演示如何优化预算的 Python 教程。
...
这句著名的话（我想是来自约翰·华纳梅克？！）说明了营销中的挑战和机遇。虽然现代分析已经取得了长足的进步，但挑战仍然存在：理解你营销预算的哪些部分能够带来价值。

由于几个因素，营销渠道在性能和投资回报率方面可能存在显著差异：

+   **受众覆盖和参与度** – 一些渠道在接触与目标受众对齐的特定潜在客户方面更为有效。

+   **获取成本** – 通过不同渠道接触潜在客户的花费各不相同。

+   **渠道饱和** – 过度使用营销渠道可能导致收益递减。

这种可变性创造了机会，可以提出关键问题，这些问题可以改变你的营销策略：

![用户生成图像](img/1299df268b4bcabff2b2a986026a026e.png)

用户生成图像

有效的预算优化是现代营销策略的一个关键组成部分。通过利用 MMM 的输出，企业可以就如何分配资源以产生最大影响做出明智的决策。MMM 提供了关于各种渠道如何对整体销售做出贡献的见解，使我们能够识别改进和优化的机会。在接下来的章节中，我们将探讨如何将 MMM 输出转换为可操作的预算分配策略。

### 2.1 响应曲线

响应曲线可以将 MMM 的输出转换为综合形式，显示销售如何对每个营销渠道的支出做出反应。

![用户生成图像](img/e7d97302bc3a49fc984f87840cf29a3b.png)

用户生成图像

单独的响应曲线本身就非常强大，它允许我们运行假设情景。以上面的响应曲线为例，我们可以估计随着我们投入更多，社会变化对销售的贡献。我们还可以直观地看到递减回报开始产生效果的地方。但如果我们想尝试回答更复杂的假设情景，比如在固定总预算的情况下优化渠道级别预算，那该怎么办呢？这正是线性规划发挥作用的地方——让我们在下一节中探讨这个问题！

### 2.2 线性规划

线性规划是一种优化方法，可以用来在给定一些约束条件下找到线性函数的最优解。它是运筹学领域的一个非常通用的工具，但并不经常得到应有的认可。它用于解决调度、运输和资源分配问题。我们将探讨如何使用它来优化营销预算。
...
+   **不等式约束**例如，渠道级别预算在 5000 万英镑和 1500 万英镑之间
...
从零开始编写预算优化代码是一项复杂但非常有价值的练习。幸运的是，**pymc-marketing**团队已经完成了繁重的工作，提供了一个强大的框架来运行预算优化场景。在下一节中，我们将探讨他们的包如何简化预算分配流程，使其更容易为分析师所使用。

## 3.0 Python 演示

现在我们已经了解了如何使用 MMM 的输出来优化预算，让我们看看我们如何使用上一篇文章中的模型来创造多少价值！在这个演示中，我们将涵盖：
...
我们现在将重新训练第一篇文章中的模型。我们将像上次一样准备训练数据，通过以下方式：

+   将数据拆分为特征和目标。

+   为训练和超时切片创建索引。

然而，由于本文的重点不是模型校准，我们将包括需求作为控制变量，而不是需求代理。这意味着模型将非常校准良好——尽管这并不非常现实，但它将给我们一些很好的结果来展示我们如何优化预算。
...
在我们进入优化之前，让我们检查我们的模型是否拟合良好。首先，我们检查真实的贡献：
...
在我们进入预算优化之前，让我们看一下响应曲线。在**pymc-marketing**包中查看响应曲线有两种方式：

1.  直接响应曲线

1.  成本份额响应曲线

让我们从直接响应曲线开始。在直接响应曲线中，我们简单地创建了一个散点图，显示了每个渠道每周支出与每周贡献之间的关系。
...
成本分担响应曲线是比较渠道有效性的另一种方式。当δ = 1.0 时，渠道支出保持在训练数据相同的水平。当δ = 1.2 时，渠道支出增加了 20%。
...
响应曲线是帮助我们在渠道级别规划未来营销预算的极好工具。接下来，让我们将它们付诸实践，运行一些预算优化场景！

### 3.5 预算优化

首先，让我们设置几个参数：

+   **perc_change**：这个参数用于设置每个渠道的最小和最大支出的约束。这个约束有助于我们保持场景的合理性，意味着我们不会将响应曲线外推得太远，超出模型在训练中看到的内容。

+   **budget_len**：这是预算场景的周数长度。

我们将首先使用预算场景期望的长度来选择最近的数据时期。
...
然后，我们使用这个最近时期来设定整体预算约束和每周级别的渠道约束：
...
现在是运行我们的场景的时候了！我们输入相关数据和参数，然后得到最优支出。我们将其与将总预算按当前预算分配比例分割（我们称之为实际支出）进行比较。
...
我们可以看到建议是将预算从数字渠道转移到电视。但这对销售额有什么影响？

为了计算最优支出的贡献，我们需要输入每个渠道的新支出值以及模型中的任何其他变量。我们只有需求，所以我们输入最近时期的平均值。我们也将以同样的方式计算平均支出的贡献。
...
最优支出使我们销售额提高了 6%！这很令人印象深刻，尤其是考虑到我们已经固定了整体预算！

## 结束语

今天，我们看到了预算优化有多么强大。它可以帮助组织进行月度/季度/年度预算规划和预测。像往常一样，提出良好建议的关键在于拥有一个稳健、校准良好的模型。

* * *

我希望你喜欢第三部分！这就是本系列关于掌握 MMM 的最后一部分。然而，如果你想了解衡量长期品牌建设效果的复杂主题，请继续关注！
