...
> 原文：[https://towardsdatascience.com/multiple-linear-regression-math-explained-simply-part-1/](https://towardsdatascience.com/multiple-linear-regression-math-explained-simply-part-1/)
...
与仅使用Python应用算法不同，在这篇博客中，**让我们探索多元线性回归算法背后的数学原理**。
...
我们将从这个数据集中考虑一个20个点的样本。

![图片](../Images/58b52f696d5c506d69dc88d2e88efe12.png)

图片由作者提供

我们从鱼市场数据集中考虑了一个20个点的样本，它包括20条鱼的测量值，特别是它们的高度和宽度以及相应的重量。这三个值将帮助我们理解多元线性回归在实际中的应用。

首先，让我们使用Python在我们的20个点样本数据上拟合一个多元线性回归模型。
...
我们使用Python在我们的样本数据集上应用了多元线性回归，并得到了结果。
...
![图片](../Images/54fbf8dc7c2a558109483c93187774b6.png)
...
![](../Images/78381a774bf64758bcd390d3046359ee.png)
...
![](../Images/011a92ab621e27bbf818d22761ce4271.png)
...
![](../Images/ba51943c0abbf0cfc0cd6e72fd651126.png)

作者提供的GIF

现在，让我们看看当我们改变斜率 β₁ 和 β₂ 时会发生什么。

![](../Images/537553a46dab03ccf8dd4baee0f233e3.png)

作者提供的GIF

![](../Images/05dc432d56078d3eec6c683daca99485.png)
...
![](../Images/538d804c51c98c80c9f9f4e07733016f.png)

作者图片

让我们绘制这些值

![](../Images/7065c04ef4817f44e7489d889cfc9e51.png)
...
m = \frac{y_2 – y_1}{x_2 – x_1} = \frac{\text{y的变化量}}{\text{x的变化量}}
...
如果我们考虑任何两个点并计算斜率，这个值将保持不变，这意味着y相对于x的变化量在整个线上是相同的。

* * *

现在，让我们考虑方程y=x²。

![](../Images/8ccfaee329677cca27678328ae513dad.png)

作者图片

让我们绘制这些值

![](../Images/1391d17c96d85baf730d00ae6e336a84.png)
...
我们可以观察到斜率是连续变化的，这意味着y相对于x的变化率在整个曲线上并不相同。
...
首先，让我们考虑x轴上的一个点x，以及距离它h个单位的另一个点，即点x+h。

对于这些x值，相应的y坐标将是f(x)和f(x+h)，因为y是x的函数。
...
![](../Images/59a1246154d33d3e6f4a251ce0a9bdff.png)

作者提供的图像

![](../Images/94a39650d91f38a705a0bd19393302aa.png)
...
例如，在曲线y=x²上，当x=2时，斜率是2x=2×2=4。

在这一点上，我们在曲线上有坐标(2,4)，该点的斜率是4。

这意味着在**那个确切点**，对于x的每1个单位变化，y会有4个单位的变化。
...
![](../Images/3640a39af8829a9e82020aaa291f35cc.png)
...
这里，这对应于找到β₀、β₁和β₂的值，使得SSR最小。这发生在SSR相对于每个系数的导数等于零时。

换句话说，在这个点上，即使β₀、β₁或β₂有轻微的变化，SSR也不会改变，这表明我们已经达到了成本函数的最小点。

* * *

简单来说，我们可以说，在我们的例子y=x²中，我们在x=0时得到了导数（斜率）2x=0，在该点，y是最小的，在这种情况下是零。
...
\textbf{损失：}\quad L(\beta_0,\beta_1,\beta_2)=\sum_{i=1}^{n}\big(y_i-\beta_0-\beta_1 x_{i1}-\beta_2 x_{i2}\big)^2

\]

\[

\textbf{令 } e_i = y_i-\beta_0-\beta_1 x_{i1}-\beta_2 x_{i2}\quad\Rightarrow\quad L=\sum e_i^2.
...
\quad\text{(链式法则：} \frac{d}{d\theta}u^2=2u\,\frac{du}{d\theta}\text{)}
...
\sum x_{i1}y_i – \beta_0\sum x_{i1} – \beta_1\sum x_{i1}^2 – \beta_2\sum x_{i1}x_{i2}=0.
...
\sum x_{i2}y_i – \beta_0\sum x_{i2} – \beta_1\sum x_{i1}x_{i2} – \beta_2\sum x_{i2}^2=0.
...
\sum x_{i1}y_i – \beta_0\sum x_{i1} – \beta_1\sum x_{i1}^2 – \beta_2\sum x_{i1}x_{i2} = 0 \quad (2)

\] \[

\sum x_{i2}y_i – \beta_0\sum x_{i2} – \beta_1\sum x_{i1}x_{i2} – \beta_2\sum x_{i2}^2 = 0 \quad (3)
...
它在Kaggle上公开发布，并受[**创意共享零（CC0公共领域）**](https://creativecommons.org/publicdomain/zero/1.0/)许可协议的许可。这意味着它可以无限制地免费使用、修改和共享，无论是**非商业用途**还是**商业用途**。
